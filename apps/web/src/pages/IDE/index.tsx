...
// ══════════════════════════════════════════════════════════════
//  NOTEBOOK — ephemeral scratchpad (no persistence)
// ══════════════════════════════════════════════════════════════

const NB_LANG_META:any = {
  js:       { color:'#ffc410', caret:'#ffc410' },
  python:   { color:'#4fc3f7', caret:'#4fc3f7' },
  markdown: { color:'#ce93d8', caret:'#ce93d8' },
}

const AC_WORDS: Record<string,string[]> = {
  js: [
    'function','const','let','var','return','if','else','for','while','in','of','class',
    'import','export','from','default','new','this','true','false','null','undefined',
    'try','catch','finally','async','await','typeof','instanceof','break','continue',
    'switch','case','throw','delete','void','static','extends','super',
    'Promise','Array','Object','Math','JSON','console','document','window',
    'fetch','setTimeout','setInterval','clearTimeout','clearInterval',
    'parseInt','parseFloat','isNaN','isFinite','Map','Set','Date','Error','RegExp',
    'console.log','console.error','console.warn','console.table','console.info',
    'Object.keys','Object.values','Object.entries','Object.assign','Object.freeze',
    'Array.from','Array.isArray','Array.of','JSON.parse','JSON.stringify',
    'Math.floor','Math.ceil','Math.round','Math.random','Math.max','Math.min','Math.abs',
  ],
  python: [
    'def','class','import','from','return','if','elif','else','for','while','in','not',
    'and','or','True','False','None','pass','break','continue','try','except','finally',
    'with','as','yield','lambda','self','raise','del','global','nonlocal','assert','async','await',
    'len','range','type','str','int','float','list','dict','set','tuple','bool',
    'map','filter','zip','enumerate','open','super','object','abs','max','min','sum',
    'sorted','reversed','print','input','repr','isinstance','hasattr','getattr','setattr',
    'vars','dir','format','round','divmod','pow','hex','oct','bin','ord','chr',
    'any','all','next','iter','hash','id','callable','staticmethod','classmethod','property',
  ],
  markdown: [],
}

function caretOffset(ta: HTMLTextAreaElement) {
  const cs = getComputedStyle(ta)
  const mirror = document.createElement('div')
  Object.assign(mirror.style, {
    position:'absolute', top:'0', left:'0', visibility:'hidden', pointerEvents:'none',
    whiteSpace:'pre-wrap', wordBreak:'break-word', width:ta.clientWidth+'px',
  ...
  const mr = mark.getBoundingClientRect(), tr = ta.getBoundingClientRect()
  mirror.remove()
  return { top: mr.top - tr.top + ta.scrollTop, left: mr.left - tr.left }
}

function _renderCellOutput(output:any[]) {
  return output.map((entry:any, i:number) => {
    const text:string = entry.val || ''
    const isErr = entry.type === 'error' || entry.type === 'run-err' || entry.type === 'compile-err'
    if (isErr) {
      return (
        <div key={i} style={{ color:'#ff6b7a', whiteSpace:'pre-wrap', wordBreak:'break-word',
          borderLeft:'2px solid rgba(255,67,90,.35)', paddingLeft:8, marginBottom:2, lineHeight:1.6 }}>
          {text}
        </div>
      )
    }
    if (entry.type === 'return' || entry.type === 'log') {
      ...
      if ((t.startsWith('{') || t.startsWith('[')) && t.length > 4) {
        try {
          return (
            <pre key={i} style={{ color:'#c792ea', margin:'0 0 2px',
    ...
    const col:any = {
      log:'#c0c8d8', warn:'#ffc410', error:'#ff6b7a', info:'#4285f4',
      return:'#a3e8c4', 'compile-warn':'#ffc410', 'compile-ok':'#10b981',
    }[entry.type] || '#c0c8d8'
    return <div key={i} style={{ color:col, whiteSpace:'pre-wrap', wordBreak:'break-word', lineHeight:1.6 }}>{text}</div>
  })
}

function NoteCell({ cell, onRun, onDelete, onCodeChange, onLangChange, onClearOutput }:any) {
  const taRef = useRef<any>(null)
  const [ac, setAc] = useState<{list:string[],idx:number,word:string,top:number,left:number}|null>(null)
  ...
  const applyAc = (word:string) => {
    const ta = taRef.current; if (!ta || !ac) return
    const s = ta.selectionStart
    const before = ta.value.slice(0, s - ac.word.length)
    const after  = ta.value.slice(s)
    onCodeChange(before + word + after)
    closeAc()
    setTimeout(() => { ta.selectionStart = ta.selectionEnd = before.length + word.length }, 0)
  }
  ...
  const handleChange = (e:any) => {
    ...
    const m = textBefore.match(/[a-zA-Z_$][\w$.]*$/)
    if (m && m[0].length >= 2) {
      const partial = m[0]
      const matches = (AC_WORDS[cell.lang] || []).filter(w => w.startsWith(partial) && w !== partial)
      if (matches.length) {
        const pos = caretOffset(ta)
        setAc({ list: matches.slice(0, 8), idx: 0, word: partial, top: pos.top + LH, left: pos.left })
      } else closeAc()
    } else closeAc()
  }
  ...
  const handleKeyDown = (e:any) => {
    if (ac) {
      if (e.key === 'ArrowDown')  { e.preventDefault(); setAc(a => a && ({...a, idx:(a.idx+1)%a.list.length})); return }
      if (e.key === 'ArrowUp')    { e.preventDefault(); setAc(a => a && ({...a, idx:(a.idx-1+a.list.length)%a.list.length})); return }
      if (e.key === 'Tab')        { e.preventDefault(); applyAc(ac.list[ac.idx] ?? ''); return }
      if (e.key === 'Escape')     { closeAc(); return }
    }
    if (e.key === 'Enter' && e.shiftKey) { e.preventDefault(); onRun(); return }
    if (e.key === 'Tab' && !ac) {
      e.preventDefault()
      const ta = e.target, s = ta.selectionStart
      onCodeChange(ta.value.slice(0, s) + '  ' + ta.value.slice(ta.selectionEnd))
      setTimeout(() => { ta.selectionStart = ta.selectionEnd = s + 2 }, 0)
    }
  }

  const statusColor = cell.status === 'running' ? '#ffc410'
    : cell.status === 'ok'    ? '#10b981'
    : cell.status === 'error' ? '#ff435a'
    : 'rgba(255,255,255,.06)'

  return (
    <div style={{ borderBottom:'1px solid rgba(255,255,255,.05)', borderLeft:`2px solid ${statusColor}`, transition:'border-color .25s' }}>

      {/* ── Cell header ── */}
      <div style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 8px 4px 10px', background:'rgba(0,0,0,.38)' }}>
        {/* Language pills */}
        <div style={{ display:'flex', gap:3 }}>
          {(['js','python','markdown'] as const).map(l => {
            const m = NB_LANG_META[l]
            const active = cell.lang === l
            return (
              <button key={l} onClick={() => onLangChange(l)} style={{
                background: active ? `${m.color}18` : 'transparent',
                border: `1px solid ${active ? m.color+'45' : 'rgba(255,255,255,.07)'}`,
                color: active ? m.color : 'rgba(200,200,220,.2)',
                fontFamily:"'Oswald',sans-serif", fontWeight:700, fontSize:'8px',
                letterSpacing:'.1em', padding:'1px 6px', cursor:'pointer', transition:'all .12s',
              }}>
                {l === 'python' ? 'PY' : l === 'markdown' ? 'MD' : 'JS'}
              </button>
            )
          })}
        </div>
        <div style={{ flex:1 }}/>
        {cell.status === 'running' && (
          <span style={{ fontSize:'8px', color:'#ffc410', fontFamily:"'Share Tech Mono',monospace" }}>●</span>
        )}
        {cell.execMs != null && cell.status !== 'idle' && cell.status !== 'running' && (
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'8px',
            color: cell.status==='error' ? '#ff435a' : 'rgba(200,200,220,.2)' }}>
            {cell.execMs}ms
          </span>
        )}
        <button onClick={onRun} title="Run · Shift+Enter" style={{
          background:`${meta.color}10`, border:`1px solid ${meta.color}30`, color:meta.color,
          fontFamily:"'Oswald',sans-serif", fontWeight:700, fontSize:'8px',
          letterSpacing:'.1em', padding:'1px 8px', cursor:'pointer', transition:'background .1s',
        }}
        onMouseEnter={e=>(e.currentTarget.style.background=`${meta.color}25`)}
        onMouseLeave={e=>(e.currentTarget.style.background=`${meta.color}10`)}>
          ▶ RUN
        </button>
        <button onClick={onDelete} style={{
          background:'transparent', border:'none', cursor:'pointer',
          color:'rgba(200,200,220,.12)', fontSize:'14px', lineHeight:1, padding:'0 2px', transition:'color .1s',
        }}
      ...
      {cell.lang === 'markdown' ? (
        <div>
          <textarea value={cell.code} onChange={(e:any)=>onCodeChange(e.target.value)}
            onKeyDown={handleKeyDown} rows={Math.max(3, lines.length)} spellCheck={false}
            placeholder="# Markdown"
            style={{ display:'block', width:'100%', boxSizing:'border-box', background:'#040410',
              border:'none', outline:'none', resize:'none',
              fontFamily:"'JetBrains Mono',monospace", fontSize:'12px', lineHeight:LH+'px',
              color:'#9ba8c4', padding:'8px 12px', caretColor:meta.caret }}/>
          {cell.code.trim() && (
            <div className="md-preview" style={{ padding:'10px 16px',
              borderTop:`1px solid ${meta.color}15`, background:'#040410', fontSize:'13px', minHeight:36 }}
              dangerouslySetInnerHTML={{ __html: renderMd(cell.code || '') }}/>
          )}
        </div>
      ) : (
        <div style={{ position:'relative', background:'#03030c' }}>
          {/* Line numbers */}
          <div aria-hidden style={{
            position:'absolute', left:0, top:0, bottom:0, width:32, overflow:'hidden',
            fontFamily:"'JetBrains Mono',monospace", fontSize:'10px', lineHeight:LH+'px',
            color:`${meta.color}22`, textAlign:'right', padding:'8px 5px 8px 0',
            userSelect:'none', pointerEvents:'none',
            background:'rgba(0,0,0,.22)', borderRight:`1px solid ${meta.color}10`,
          }}>
          ...
          <pre aria-hidden style={{
            ...
            fontFamily:"'JetBrains Mono',monospace", fontSize:'12px', lineHeight:LH+'px',
            pointerEvents:'none', overflow:'hidden', whiteSpace:'pre-wrap', wordBreak:'break-word',
          }} dangerouslySetInnerHTML={{ __html: highlightCode(cell.code || '') }}/>
          {/* Transparent editable textarea */}
          <textarea ref={taRef} value={cell.code}
            onChange={handleChange} onKeyDown={handleKeyDown} onBlur={closeAc}
            rows={Math.max(3, lines.length)} spellCheck={false}
            style={{
              display:'block', width:'100%', boxSizing:'border-box',
              background:'transparent', border:'none', outline:'none', resize:'none',
              fontFamily:"'JetBrains Mono',monospace", fontSize:'12px', lineHeight:LH+'px',
              color:'transparent', caretColor:meta.caret,
              padding:`8px 10px 8px 42px`, minHeight:Math.max(3,lines.length)*LH+16+'px',
            }}/>
          {/* Autocomplete popup */}
          {ac && ac.list.length > 0 && (
            <div style={{
              position:'absolute', top:ac.top, left:Math.min(ac.left, 180),
              zIndex:200, minWidth:150, maxWidth:260,
              background:'#0c0c1e', border:'1px solid rgba(255,255,255,.1)',
              boxShadow:'0 8px 32px rgba(0,0,0,.9)',
            }}>
              {ac.list.map((w, i) => (
                <div key={w} onMouseDown={(e:any)=>{ e.preventDefault(); applyAc(w) }} style={{
                  padding:'4px 10px', display:'flex', gap:0,
                  fontFamily:"'JetBrains Mono',monospace", fontSize:'11px',
                  color: i===ac.idx ? meta.color : '#6878a0',
                  background: i===ac.idx ? `${meta.color}10` : 'transparent',
                  cursor:'pointer',
                }}>
                  <span style={{ opacity:.4 }}>{ac.word}</span>
                  <span style={{ fontWeight:700 }}>{w.slice(ac.word.length)}</span>
                </div>
              ))}
              <div style={{ padding:'2px 10px 3px', borderTop:'1px solid rgba(255,255,255,.05)',
                fontFamily:"'Share Tech Mono',monospace", fontSize:'8px',
                color:'rgba(200,200,220,.18)', letterSpacing:'.04em' }}>
      ...
      {cell.output.length > 0 && (
        <div style={{ background:'#020209', borderTop:`1px solid ${cell.status==='error'?'rgba(255,67,90,.18)':`${meta.color}10`}` }}>
          <div style={{ display:'flex', alignItems:'center', padding:'3px 8px 3px 12px',
            borderBottom:'1px solid rgba(255,255,255,.04)' }}>
            <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'8px', opacity:.5,
              color: cell.status==='error' ? '#ff435a' : '#10b981' }}>
              {cell.status==='error' ? '✗ error' : '✓ out'}
            </span>
            <div style={{ flex:1 }}/>
            <button onClick={() => navigator.clipboard.writeText(cell.output.map((e:any)=>e.val).join('\n')).catch(()=>{})}
              title="Copy" style={{ background:'transparent', border:'none', cursor:'pointer',
                color:'rgba(200,200,220,.12)', fontSize:'10px', transition:'color .1s', padding:'0 4px' }}
              onMouseEnter={e=>(e.currentTarget.style.color='#10b981')}
              onMouseLeave={e=>(e.currentTarget.style.color='rgba(200,200,220,.12)')}>⎘</button>
            <button onClick={onClearOutput} style={{ background:'transparent', border:'none', cursor:'pointer',
              color:'rgba(200,200,220,.1)', fontSize:'12px', lineHeight:1, padding:'0 3px', transition:'color .1s' }}
              onMouseEnter={e=>(e.currentTarget.style.color='rgba(200,200,220,.4)')}
              onMouseLeave={e=>(e.currentTarget.style.color='rgba(200,200,220,.1)')}>×</button>
          </div>
          <div style={{ padding:'6px 10px 8px 14px', maxHeight:180, overflowY:'auto',
            fontFamily:"'JetBrains Mono',monospace", fontSize:'11px', lineHeight:1.6,
...
function NotebookPanel({ brutal }:any) {
  ...
  const runCell = useCallback(async (cellId:string) => {
    ...
    if (!cell || cell.lang === 'markdown') return
    const t0 = performance.now()
    setCells(cs => cs.map((c:any) => c.id===cellId ? {...c, status:'running', output:[], execCount:++execCounterRef.current, execMs:null} : c))
    const result = cell.lang === 'python' ? await runPython(cell.code) : await runJS(cell.code)
    const ms = Math.round(performance.now() - t0)
    setCells(cs => cs.map((c:any) => c.id===cellId ? {...c, status:result.error?'error':'ok', output:result.logs, execMs:ms} : c))
  }, [cells])

  const runAll = async () => { for (const c of cells) if (c.lang !== 'markdown') await runCell(c.id) }

  const addCell = (lang = 'js') =>
    setCells(cs => [...cs, { id:'nb'+Date.now(), lang, code:'', output:[], status:'idle', execCount:null, execMs:null }])

  const langBtns = [
    { lang:'js',       color:'#ffc410', label:'+ JS' },
    { lang:'python',   color:'#4fc3f7', label:'+ PY' },
    { lang:'markdown', color:'#ce93d8', label:'+ MD' },
  ]

  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#03030c', overflow:'hidden' }}>

      {/* ── Toolbar ── */}
      <div style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 8px', flexShrink:0,
        borderBottom:'1px solid rgba(255,255,255,.06)', background:'rgba(0,0,0,.5)' }}>
        <span style={{ fontFamily:"'Oswald',sans-serif", fontWeight:700, fontSize:'8px',
          letterSpacing:'.2em', color:'rgba(200,200,220,.18)', marginRight:2 }}>SCRATCH</span>
        <div style={{ width:1, height:10, background:'rgba(255,255,255,.06)', marginRight:2 }}/>
        {langBtns.map(b => (
          <button key={b.lang} onClick={() => addCell(b.lang)} style={{
            background:`${b.color}0e`, border:`1px solid ${b.color}25`,
            color:b.color, fontFamily:"'Oswald',sans-serif", fontWeight:700, fontSize:'8px',
            letterSpacing:'.1em', padding:'1px 7px', cursor:'pointer', transition:'background .1s', lineHeight:1.8,
          }}
          onMouseEnter={e=>(e.currentTarget.style.background=`${b.color}22`)}
          onMouseLeave={e=>(e.currentTarget.style.background=`${b.color}0e`)}>
            {b.label}
          </button>
        ))}
        <div style={{ flex:1 }}/>
        {cells.length > 0 && (
          <button onClick={runAll} style={{
            ...
            color:'#10b981', fontFamily:"'Oswald',sans-serif", fontWeight:700, fontSize:'8px',
            letterSpacing:'.1em', padding:'1px 7px', cursor:'pointer', transition:'background .1s', lineHeight:1.8,
          }}
          onMouseEnter={e=>(e.currentTarget.style.background='rgba(16,185,129,.2)')}
          onMouseLeave={e=>(e.currentTarget.style.background='rgba(16,185,129,.08)')}>▶ ALL</button>
        )}
        {cells.length > 0 && (
          <button onClick={() => setCells(cs => cs.map((c:any) => ({...c, output:[], status:'idle', execMs:null})))}
            style={{ background:'transparent', border:'none', cursor:'pointer',
              fontFamily:"'Oswald',sans-serif", fontSize:'8px', letterSpacing:'.1em',
              color:'rgba(200,200,220,.18)', padding:'1px 5px', lineHeight:1.8, transition:'color .1s' }}
            onMouseEnter={e=>(e.currentTarget.style.color='rgba(200,200,220,.55)')}
            onMouseLeave={e=>(e.currentTarget.style.color='rgba(200,200,220,.18)')}>CLR</button>
        )}
      </div>
      ...
      {cells.length === 0 ? (
        <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center',
          ...
          <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'10px',
            color:'rgba(200,200,220,.18)', letterSpacing:'.08em' }}>scratch pad · nothing stored</div>
          <div style={{ display:'flex', gap:6 }}>
            {langBtns.map(b => (
              <button key={b.lang} onClick={() => addCell(b.lang)} style={{
                background:`${b.color}0a`, border:`1px solid ${b.color}22`,
                color:b.color, fontFamily:"'Oswald',sans-serif", fontWeight:700,
                fontSize:'9px', letterSpacing:'.1em', padding:'5px 16px', cursor:'pointer', transition:'all .15s',
              }}
              onMouseEnter={e=>{e.currentTarget.style.background=`${b.color}1c`;e.currentTarget.style.borderColor=`${b.color}50`}}
              onMouseLeave={e=>{e.currentTarget.style.background=`${b.color}0a`;e.currentTarget.style.borderColor=`${b.color}22`}}>
                {b.label}
              </button>
            ))}
          </div>
          <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'9px',
            color:'rgba(200,200,220,.1)', letterSpacing:'.05em' }}>
            Shift+Enter run · Tab indent · no saves
          </div>
        </div>
      ) : (
        <div style={{ flex:1, overflowY:'auto', scrollbarWidth:'thin', scrollbarColor:'rgba(255,255,255,.06) transparent' }}>
          {cells.map((cell:any) => (
            <NoteCell key={cell.id} cell={cell}
              onRun={() => runCell(cell.id)}
              onDelete={() => setCells(cs => cs.filter((c:any) => c.id !== cell.id))}
              onCodeChange={(code:string) => setCells(cs => cs.map((c:any) => c.id===cell.id ? {...c,code} : c))}
              onLangChange={(lang:string) => setCells(cs => cs.map((c:any) => c.id===cell.id ? {...c,lang,output:[],status:'idle'} : c))}
              onClearOutput={() => setCells(cs => cs.map((c:any) => c.id===cell.id ? {...c,output:[],status:'idle',execMs:null} : c))}
            />
          ))}
          <div style={{ padding:'8px 10px', display:'flex', gap:4, borderTop:'1px solid rgba(255,255,255,.04)' }}>
            {langBtns.map(b => (
              <button key={b.lang} onClick={() => addCell(b.lang)} style={{
                background:'transparent', border:`1px dashed ${b.color}20`,
                color:`${b.color}45`, fontFamily:"'Oswald',sans-serif", fontWeight:700,
                fontSize:'8px', letterSpacing:'.1em', padding:'2px 10px',
                cursor:'pointer', transition:'all .12s',
              }}
              onMouseEnter={e=>{e.currentTarget.style.background=`${b.color}0c`;e.currentTarget.style.color=b.color}}
              onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color=`${b.color}45`}}>
                {b.label}
              </button>
            ))}
          </div>
