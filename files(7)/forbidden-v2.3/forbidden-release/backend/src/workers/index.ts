...
export const timelineWorker = new Worker(
  ...
  async (job) => {
    ...
    io.to(`workspace:${workspaceId}`).emit('timeline:update', entry.toObject());
  },
...
export const containerSyncWorker = new Worker(
  ...
  async (job) => {
    ...
    switch (event.type) {
      ...
      case 'NODE_DELETED': {
        await deleteNodeFromContainer(workspaceId, nodeId);
        break;
      }
      case 'NOTE_SAVED': {
...
export const reaperQueue = new Queue('idle-reaper', { connection: { ...connection } });
...
export const snapshotQueue = new Queue('snapshot', { connection: { ...connection } });
...
export const activityFeedQueue = new Queue('activity-feed', { connection: { ...connection } });
...
export const ptyOutputQueue = new Queue('pty-output', { connection: { ...connection } });
