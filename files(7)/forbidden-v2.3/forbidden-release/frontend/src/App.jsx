// Extracted from legacy index-final.html during React app migration.
...
// ── API config ──────────────────────────────────────────────────────────────
const API_URL = window.FORBIDDEN_API_URL || 'http://localhost:3001';
...
function normalizeApiNodes(data) {
  if (Array.isArray(data)) return data;
  if (Array.isArray(data?.nodes)) return data.nodes;
  return [];
}

function buildNodeTemplate(rawName, type) {
  const slug = rawName
    .trim()
    .replace(/[^\w\s-]/g, '')
    .replace(/[\s-]+/g, '_')
  ...
  if (type === 'entry') {
    ...
    code = `def ${slug}():\n    result = []\n    return result`;
  } else if (type === 'util' || type === 'helper') {
    code = `def ${slug}(*args, **kwargs):\n    return None`;
  }
...
const PALETTES = [
  // ── DARK THEMES ──
  { id:'forbidden',  name:'FORBIDDEN',    bg:'#0b0b0f', base:'#c0c8d8', lineNum:'#2e2e42', activeLine:'rgba(255,255,255,0.035)', kw:'#ff435a', str:'#ffc410', cmt:'#3e3e5a', num:'#4285f4', fn:'#10b981', bi:'#28f1c3', op:'#6a6a8a', swatches:['#ff435a','#ffc410','#10b981','#28f1c3'] },
...
const TERM_PALETTES = [
  ...
  { id:'forbidden', name:'FORBIDDEN',     bg:'#080810', text:'#c0c8d8', prompt:'#10b981', dim:'#3e3e5a', error:'#ff435a', warn:'#ffc410', info:'#28f1c3', border:'#1a1a2c', cursor:'#10b981', selection:'rgba(16,185,129,0.15)' },
...
function paletteToMonacoTheme(palette) {
  const light = palette.bg.startsWith('#f') || palette.bg.startsWith('#FF') || palette.bg.startsWith('#FE') || parseInt(palette.bg.slice(1,3),16) > 200;
  return { base: light ? 'vs' : 'vs-dark', inherit: true,
    rules: [
      { token:'keyword', foreground: palette.kw.replace('#','') },
      { token:'string', foreground: palette.str.replace('#','') },
      { token:'comment', foreground: palette.cmt.replace('#','') },
      { token:'number', foreground: palette.num.replace('#','') },
    ],
    colors: {
      'editor.background': palette.bg,
      'editor.foreground': palette.base,
      'editorLineNumber.foreground': palette.lineNum,
      'editorLineNumber.activeForeground': palette.base,
      'editor.lineHighlightBackground': palette.activeLine,
      'editorCursor.foreground': palette.fn,
      'editor.selectionBackground': palette.fn + '33',
    }
  };
}
function detectLanguage(label='') {
  const ext = (label.split('.').pop()||'').toLowerCase();
  return {py:'python',js:'javascript',ts:'typescript',jsx:'javascript',tsx:'typescript',rs:'rust',go:'go',java:'java',cpp:'cpp',c:'c',cs:'csharp',rb:'ruby',html:'html',css:'css',json:'json',md:'markdown',sh:'shell',yaml:'yaml',yml:'yaml',sql:'sql'}[ext]||'plaintext';
}

function CodeEditor({ node, onChange, externalPalette, onRun, execOutput, execStatus, ydoc, awareness }) {
  ...
  // Monaco find/replace — operates on model directly
  const handleFindReplace = () => {
    const editor = monacoEditorRef.current;
    if (!editor || !findQuery) return;
    const val = editor.getValue();
    const count = val.split(findQuery).length - 1;
    editor.setValue(val.split(findQuery).join(replaceQuery));
  ...
  const monacoBindingRef = useRef(null); // y-monaco CRDT binding
  ...
  useEffect(() => {
    ...
    loadMonaco().then(monaco => {
      ...
      const lang = detectLanguage(node.label);
      const themeName = 'forbidden-' + palette.id;
      monaco.editor.defineTheme(themeName, paletteToMonacoTheme(palette));
      const model = monaco.editor.createModel(node.code || '', lang);
      monacoModelRef.current = model;
      const editor = monaco.editor.create(monacoContainerRef.current, {
        model, theme: themeName, fontSize, fontFamily: "'JetBrains Mono', monospace",
        minimap: { enabled: minimap }, wordWrap: wordWrap ? 'on' : 'off',
        lineNumbers: 'on', scrollBeyondLastLine: false, renderWhitespace: 'selection',
        automaticLayout: true, contextmenu: true, smoothScrolling: true,
        cursorBlinking: 'smooth', cursorSmoothCaretAnimation: 'on',
        renderLineHighlight: 'all', scrollbar: { verticalScrollbarSize: 5, horizontalScrollbarSize: 5 },
      ...
      // Override Ctrl+S to mark node saved
      ...
      // Phase 2.1 — y-monaco CRDT binding
      // If a Yjs doc is provided (live backend), bind Monaco to a YText shared type.
      // Text edits then flow through Yjs rather than manual onChange → node:edit.
      const MonacoBinding = window.MonacoBinding; // loaded from y-monaco CDN
      if (ydoc && MonacoBinding) {
        const ytext = ydoc.getText(`node:${node.id}`);
        // Only set initial content if ytext is empty (first editor to open this node)
        if (ytext.toString() === '' && node.code) {
          ydoc.transact(() => { ytext.insert(0, node.code); });
        }
        const editors = new Set([editor]);
        const binding = new MonacoBinding(ytext, model, editors, awareness || undefined);
        monacoBindingRef.current = binding;
        // CRDT drives text — only emit node:edit for metadata (on ctrl+S), not every keystroke
      } else {
        // Fallback: manual debounced emit (no Yjs)
        let editTimer;
        editor.onDidChangeModelContent(() => {
          const val = editor.getValue();
          clearTimeout(editTimer);
          editTimer = setTimeout(() => onChange(val), 300);
        });
      }
      setMonacoReady(true);
    });
    return () => {
      ...
      monacoEditorRef.current = null; monacoModelRef.current = null;
  ...
  useEffect(() => {
    ...
    const themeName = 'forbidden-' + palette.id;
    window.monaco.editor.defineTheme(themeName, paletteToMonacoTheme(palette));
    window.monaco.editor.setTheme(themeName);
  }, [palette.id, monacoReady]);

  useEffect(() => { monacoEditorRef.current?.updateOptions({ fontSize, minimap: { enabled: minimap }, wordWrap: wordWrap ? 'on' : 'off' }); }, [fontSize, minimap, wordWrap]);
  ...
  const handleCopy = () => { navigator.clipboard.writeText(monacoEditorRef.current?.getValue() || node.code || '').catch(()=>{}); showToast('COPIED'); };
  const handleFormat = () => { const formatAction = monacoEditorRef.current?.getAction('editor.action.formatDocument'); if (formatAction) { formatAction.run(); showToast('FORMATTED'); } else { const val = (monacoEditorRef.current?.getValue()||node.code||'').split('\n').map(l=>l.replace(/\s+$/,'')).join('\n').replace(/\n{3,}/g,'\n\n'); monacoEditorRef.current?.setValue(val); onChange(val); showToast('FORMATTED'); } };
  ...
  return (
    <div className="editor-palette-scope" style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, overflow:'hidden', background:palette.bg }}>
      <div className="editor-toolbar">
        ...
        {onRun && (
          <>
            <button className={`editor-toolbar-btn ${execStatus==='running'?'active':''}`}
              style={{color:execStatus==='running'?'#ffc410':execStatus==='success'?'#10b981':execStatus==='error'?'#ff435a':'inherit',borderColor:execStatus==='running'?'rgba(255,196,16,0.4)':execStatus==='success'?'rgba(16,185,129,0.4)':execStatus==='error'?'rgba(255,67,90,0.4)':'rgba(128,128,128,0.15)',fontWeight:'bold'}}
              onClick={()=>{onRun(node.id);setShowOutput(true);}} disabled={execStatus==='running'}>
              {execStatus==='running'?'▶ RUNNING':execStatus==='success'?'✓ RUN':execStatus==='error'?'✗ RUN':'▶ RUN'}
            </button>
            <button className={`editor-toolbar-btn ${showOutput?'active':''}`} onClick={()=>setShowOutput(v=>!v)} style={{fontSize:'9px',padding:'4px 7px'}}>OUT</button>
        ...
        <button className="editor-toolbar-btn" onClick={() => monacoEditorRef.current?.getAction('editor.action.toggleLineComment')?.run()}># COMMENT</button>
        <div className="editor-toolbar-sep"/>
      ...
      <div style={{display:'flex',flex:1,overflow:'hidden',minHeight:0,flexDirection:'column'}}>
        ...
        {/* Diff panel */}
        {showDiff && (
          <div style={{width:'220px',flexShrink:0,borderRight:`1px solid ${palette.lineNum}44`,overflow:'auto',background:palette.bg,display:'flex',flexDirection:'column'}}>
        ...
        <div style={{flex:1,position:'relative',overflow:'hidden',minWidth:0}}>
          {!monacoReady && (
            <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:palette.bg,zIndex:2}}>
              <div style={{fontSize:'10px',opacity:0.3,letterSpacing:'2px',fontFamily:"'JetBrains Mono',monospace"}}>LOADING MONACO...</div>
            </div>
          )}
        ...
        <div className="code-panel-info">
          ...
          <div className="code-panel-info-body">
            <div>
              ...
              <div style={{opacity:0.4,marginTop:'2px',fontSize:'9px'}}>{node.type} · {detectLanguage(node.label)}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div style={{fontSize:'9px',opacity:0.35,lineHeight:'2',display:'flex',flexDirection:'column',gap:'1px'}}>
                <span>^S — save</span><span>^/ — comment</span><span>F1 — command</span><span>^Space — suggest</span>
              </div>
            </div>
            <div>
              ...
              {execStatus && execStatus !== 'idle' && (
                <div style={{marginTop:'8px',display:'flex',alignItems:'center',gap:'5px'}}>
                  <div style={{width:'6px',height:'6px',borderRadius:'50%',background:execStatus==='running'?'#ffc410':execStatus==='success'?'#10b981':'#ff435a',boxShadow:`0 0 6px ${execStatus==='running'?'#ffc410':execStatus==='success'?'#10b981':'#ff435a'}`}}/>
                  <span style={{fontSize:'9px',fontWeight:'bold',color:execStatus==='running'?'#ffc410':execStatus==='success'?'#10b981':'#ff435a',textTransform:'uppercase'}}>{execStatus}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {showOutput && (
          <div style={{height:'160px',flexShrink:0,borderTop:'1px solid rgba(255,255,255,0.06)',background:'rgba(0,0,0,0.5)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            <div style={{display:'flex',alignItems:'center',gap:'8px',padding:'4px 10px',borderBottom:'1px solid rgba(255,255,255,0.04)',flexShrink:0}}>
              <span style={{fontSize:'9px',opacity:0.35,letterSpacing:'1.5px'}}>OUTPUT</span>
              <span style={{fontSize:'9px',padding:'1px 6px',borderRadius:'3px',background:execStatus==='running'?'rgba(255,196,16,0.15)':execStatus==='success'?'rgba(16,185,129,0.15)':execStatus==='error'?'rgba(255,67,90,0.15)':'transparent',color:execStatus==='running'?'#ffc410':execStatus==='success'?'#10b981':execStatus==='error'?'#ff435a':'transparent'}}>{execStatus||''}</span>
              <button onClick={()=>setShowOutput(false)} style={{marginLeft:'auto',background:'transparent',border:'none',color:'inherit',opacity:0.3,cursor:'pointer',fontSize:'11px'}}>✕</button>
            </div>
            <pre style={{flex:1,overflow:'auto',padding:'8px 12px',margin:0,fontSize:'10px',fontFamily:"'JetBrains Mono',monospace",lineHeight:'1.6',whiteSpace:'pre-wrap',wordBreak:'break-all',color:execStatus==='error'?'#ff8080':'rgba(200,230,200,0.85)'}}>{execOutput || (execStatus==='running'?'▋ running...':'(no output)')}</pre>
          </div>
        )}
      </div>
      ...
      {toastMsg && <div className={`copy-toast${toastMsg.includes('SAVED')||toastMsg.includes('✓')?'  save-toast':''}`}>{toastMsg}</div>}
    </div>
  );
}
...
// Demo nodes shown before connecting to a real backend
// Z_TIME_SCALE: world units per millisecond. 200 units = ~1 day apart visually.
const Z_TIME_SCALE = 200 / (24 * 60 * 60 * 1000); // 200 units per day

// Compute Z from a timestamp (ms since epoch)
function timeToZ(tsMs, epochMs) { return (tsMs - epochMs) * Z_TIME_SCALE; }

const NOW = Date.now();
const DAY = 24*60*60*1000;
const INITIAL_NODES = [
  { id:'n1', type:'entry', label:'core_sys.py', isMain:true, x:0, y:0, vx:0, vy:0, themeIdx:0, createdAt: NOW - 6*DAY, code:'import torch\nimport sys\n\nprint("Central Architecture Booted")\n\ndef init_sequence():\n    # Core logic entry\n    pass\n\nif __name__ == "__main__":\n    init_sequence()', modified:false },
  { id:'n2', type:'function', label:'load_network.py', isMain:false, x:140, y:-130, vx:0, vy:0, themeIdx:5, classId:'g1', createdAt: NOW - 4*DAY, code:'def load_network(config=None):\n    """Load the neural network from disk.\n\n    Args:\n        config: Optional configuration dict\n    Returns:\n        DataLoader instance\n    """\n    loader = DataLoader(config)\n    loader.init()\n    return loader\n\n# Network params\nDEFAULT_LR = 0.001\nDEFAULT_BATCH = 32', modified:true },
  { id:'n3', type:'class', label:'DataMatrix.py', isMain:false, x:-100, y:150, vx:0, vy:0, themeIdx:6, code:'class DataMatrix:\n    """Core data matrix handler."""\n\n    def __init__(self, size=128):\n        self.active = True\n        self.buffer = []\n        self.size = size\n\n    def push(self, data):\n        if len(self.buffer) < self.size:\n            self.buffer.append(data)\n            return True\n        return False\n\n    def flush(self):\n        self.buffer = []\n        return self', modified:false, classId:'g1', createdAt: NOW - 4*DAY },
  { id:'n4', type:'function', label:'preprocess.py', isMain:false, x:60, y:180, vx:0, vy:0, themeIdx:4, classId:null, createdAt: NOW - 1*DAY, code:'def preprocess(data):\n    return data', modified:false },
];
...
const CMD_ITEMS = [
  ...
  { icon:'D', label:'Toggle dark / brutal', hint:'' },
];

// ── PTY Audit Log panel ──────────────────────────────────────────────────
function AuditPanel({ token, workspaceId }) {
  const [entries, setEntries] = useState([]);
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    if (!token || !workspaceId) return;
    setLoading(true);
    apiFetch(`/api/pty-audit/${workspaceId}/commands`, {}, token)
      .then(data => { setEntries(Array.isArray(data) ? data : []); setLoading(false); })
      .catch(() => setLoading(false));
  }, [token, workspaceId]);
  return (
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      <div style={{padding:'6px 14px',borderBottom:'1px solid rgba(128,128,128,0.08)',fontSize:'9px',opacity:0.35,letterSpacing:'1px',display:'flex',justifyContent:'space-between'}}>
        <span>PTY COMMAND AUDIT LOG</span>
        {token && <button style={{fontSize:'8px',padding:'1px 6px',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'2px',background:'transparent',color:'rgba(255,255,255,0.3)',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace"}} onClick={()=>{setLoading(true);apiFetch(`/api/pty-audit/${workspaceId}/commands`,{},token).then(d=>{setEntries(Array.isArray(d)?d:[]);setLoading(false);}).catch(()=>setLoading(false));}}>REFRESH</button>}
      </div>
      <div style={{flex:1,overflowY:'auto',fontFamily:"'JetBrains Mono',monospace",fontSize:'10px'}}>
        {loading && <div style={{padding:'24px',textAlign:'center',opacity:0.3,letterSpacing:'2px'}}>LOADING...</div>}
        {!loading && entries.length === 0 && (
          <div style={{padding:'24px',textAlign:'center',opacity:0.2,fontSize:'9px',letterSpacing:'1px'}}>{token ? 'No commands recorded yet' : 'Connect to backend to view audit log'}</div>
        )}
        {entries.map((entry, i) => (
          <div key={entry._id||i} style={{display:'flex',gap:'10px',padding:'6px 14px',borderBottom:'1px solid rgba(128,128,128,0.04)',alignItems:'flex-start'}}
            onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'}
            onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
            <span style={{color:'#10b981',flexShrink:0,opacity:0.6,fontSize:'9px',marginTop:'1px',minWidth:'44px'}}>{entry.exitCode===0?'✓':'✗'} {entry.exitCode!==undefined?`[${entry.exitCode}]`:''}</span>
            <span style={{flex:1,color:'#c0c8d8',wordBreak:'break-all'}}>{entry.command||entry.cmd||entry.data}</span>
            <span style={{opacity:0.25,fontSize:'8px',flexShrink:0,marginTop:'1px'}}>{entry.startedAt?new Date(entry.startedAt).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit',second:'2-digit'}):''}</span>
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="grp-editor-body">
          ...
          <div className="grp-main">
            ...
            <div className="grp-statusbar">
              ...
              <span style={{marginLeft:'auto', opacity:0.3}}>FORBIDDEN // CLASS ASSEMBLY</span>
            </div>
          </div>
...
//  MAIN IDE
// ═══════════════════════════════════════════════════════════════
function IDE({ initialTheme, initialAvatar, token, operatorId, operatorName, workspaceId, workspaceName, onLogout }) {
  const [themeMode, setThemeMode] = useState(initialTheme);
  const [clockStr, setClockStr] = useState(() => new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}));
  useEffect(() => { const id = setInterval(() => setClockStr(new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'})), 10000); return () => clearInterval(id); }, []);
  ...
  // Phase 1.3 — container status
  const [containerStatus, setContainerStatus] = useState('unknown'); // 'unknown'|'starting'|'running'|'paused'|'dead'
  // Phase 4 — node execution state
  const [nodeExecStatus, setNodeExecStatus] = useState({}); // { [nodeId]: 'running'|'success'|'error'|'idle' }
  const [nodeOutputs, setNodeOutputs] = useState({}); // { [nodeId]: string }
  ...
  // (playhead removed — timeline now shows live event feed)
  ...
  const boardSyncRef = useRef(null); // debounce ref for card syncs

  // Phase 1.2 — load board from backend on mount
  useEffect(() => {
    if (!token || !workspaceId) return;
    setBoardLoading(true);
    apiFetch(`/api/tasks/${workspaceId}`, {}, token).then(data => {
      if (!data?.columns) return;
      // Map backend columns to frontend board shape
      // Backend cols: todo, in-progress, done, archived → map to c1-c5 layout
      const STATUS_MAP = { 'todo': 'c2', 'in-progress': 'c3', 'done': 'c5', 'archived': 'c1' };
      const allTasks = data.columns.flatMap(col => col.tasks.map(t => ({
        id: t._id || t.id,
        _backendId: t._id || t.id,
        colId: STATUS_MAP[t.status] || 'c2',
        title: t.title,
  ...
  const [joinEdgeType, setJoinEdgeType] = useState('default'); // edge type for next join
  const [hoveredEdgeId, setHoveredEdgeId] = useState(null);
  // Phase 6 — AI panel
  const [aiMessages, setAiMessages] = useState([]);
  const [aiInput, setAiInput] = useState('');
  const [aiLoading, setAiLoading] = useState(false);
  const [aiMode, setAiMode] = useState('explain'); // 'explain'|'debug'|'refactor'
  // Phase 6 — pipeline
  ...
  // (terminal state moved to xterm.js)
  ...
  // ── Socket.IO + workspace data loading ──────────────────────────────────
  useEffect(() => {
    if (!token || !workspaceId) return;

    // Load recent messages
    apiFetch(`/api/messages/${workspaceId}`, {}, token).then(data => {
      if (Array.isArray(data)) setChatMessages(data.slice(-50).map(m => ({
        id: m._id || Math.random(), from: m.operatorName || m.operatorId || 'Operator',
        time: m.createdAt ? new Date(m.createdAt).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}) : '',
        text: m.text, self: m.operatorId === operatorId,
      })));
    }).catch(() => {});

    // Load workspace nodes from API
    apiFetch(`/api/nodes/${workspaceId}`, {}, token).then(data => {
      const apiNodes = normalizeApiNodes(data);
      if (apiNodes.length > 0) {
        // Map API nodes to frontend shape
        nodesRef.current = apiNodes.map(n => ({
          id: n.id,
          label: n.label,
          type: n.type || 'function',
          isMain: n.isMain || false,
          x: n.position?.x || (Math.random() - .5) * 400,
          y: n.position?.y || (Math.random() - .5) * 300,
          vx: 0, vy: 0,
          themeIdx: n.themeIdx ?? Math.floor(Math.random() * 16),
          classId: n.groupId || null,
          code: n.code || '',
          modified: false,
        ...
        const edgeMap = [];
        apiNodes.forEach(n => {
          (n.edges || []).forEach(e => edgeMap.push({
            id: `${n.id}-${e.targetId}`,
            source: n.id,
            target: e.targetId,
        ...
        edgesRef.current = edgeMap;
        forceRender({});
      }
    }).catch(err => console.warn('[forbidden] Node load failed:', err.message));
    ...
    sock.on('connect', () => {
      setWsStatus('live');
      // Fetch container status on connect
      if (token) {
        apiFetch(`/api/workspaces/${workspaceId}/status`, {}, token)
          .then(d => setContainerStatus(d.status || 'unknown'))
          .catch(() => {});
      }
    });
    ...
    // Incoming node events from other operators
    sock.on('event:new', (event) => {
      const p = event.payload || {};
      if (event.type === 'NODE_CREATED') {
        const exists = nodesRef.current.find(n => n.id === p.nodeId);
        if (!exists) {
          nodesRef.current = [...nodesRef.current, {
            id: p.nodeId, label: p.label, type: p.type || 'function', isMain: false,
            x: p.position?.x || (Math.random()-.5)*300, y: p.position?.y || (Math.random()-.5)*300,
            z: 0, vx: 0, vy: 0, themeIdx: p.themeIdx ?? 0, classId: null, code: p.code || '', modified: false,
            language: p.language || 'python', 
            createdAt: p.createdAt ? new Date(p.createdAt).getTime() : Date.now(),
          }];
              forceRender({});
            }
          } else if (event.type === 'NODE_EDITED') {
            nodesRef.current = nodesRef.current.map(n => n.id === p.nodeId ? {
              ...n,
              code: p.code ?? n.code,
              language: p.language ?? n.language,
              x: p.position?.x ?? n.x,
              y: p.position?.y ?? n.y,
              modified: p.code !== undefined ? true : n.modified,
            } : n);
            forceRender({});
          } else if (event.type === 'NODE_DELETED') {
        nodesRef.current = nodesRef.current.filter(n => n.id !== p.nodeId);
        edgesRef.current = edgesRef.current.filter(e => e.source !== p.nodeId && e.target !== p.nodeId);
        forceRender({});
      } else if (event.type === 'NODE_JOINED') {
        const exists = edgesRef.current.find(e => e.source === p.sourceId && e.target === p.targetId);
        if (!exists) {
          edgesRef.current = [...edgesRef.current, {
            id: `${p.sourceId}-${p.targetId}`,
            source: p.sourceId,
            target: p.targetId,
            edgeType: p.edgeType || 'default',
            label: p.label || '',
          }];
          forceRender({});
        }
      } else if (event.type === 'NODE_CUT') {
        edgesRef.current = edgesRef.current.filter(e => !(e.source === p.sourceId && e.target === p.targetId));
        forceRender({});
      }
    });

    // Timeline updates
    sock.on('timeline:update', (entry) => {
      setTimelineEntries(prev => [entry, ...prev].slice(0, 200));
      setAllTimelineEvents(prev => [...prev, entry].slice(0, 500));
      // If at live pos (100), stay at live
      setScrubberPos(100);
    });

    // Incoming chat messages from other operators
    sock.on('message:new', (msg) => {
      if (msg.operatorId !== operatorId) {
        setChatMessages(m => [...m, {
          id: msg._id || Date.now(), from: msg.operatorName || msg.operatorId || 'Operator',
    ...
    sock.on('terminal:output', ({ sessionId, data }) => {
      if (sessionId === sessionIdRef.current) {
        setTermOutputQueue(q => [...q, data]);
      }
    });

    sock.on('terminal:closed', ({ sessionId }) => {
      if (sessionId === sessionIdRef.current) {
        setTermOpen(false);
        setTermOutputQueue(q => [...q, '\r\n[session closed]\r\n']);
      }
    });

    // Activity feed
    sock.on('feed:new', (entry) => {
      setFeedEntries(prev => [entry, ...prev].slice(0, 200));
      setFeedUnread(n => n + 1);
    });

    // Phase 1.3 — container status events
    sock.on('container:status', ({ status }) => setContainerStatus(status));

    // Phase 4 — node run events
    sock.on('node:run:status', ({ nodeId, status }) => {
      setNodeExecStatus(s => ({ ...s, [nodeId]: status }));
      if (status === 'running') setRunningNode(nodeId);
    });
    sock.on('node:run:output', ({ nodeId, chunk }) => {
      setNodeOutputs(o => ({ ...o, [nodeId]: (o[nodeId] || '') + chunk }));
    });
    sock.on('node:run:done', ({ nodeId, exitCode }) => {
      setNodeExecStatus(s => ({ ...s, [nodeId]: exitCode === 0 ? 'success' : 'error' }));
      setRunningNode(null);
      // Phase 4.2 — error propagation: mark downstream nodes as warned
      if (exitCode !== 0) {
        const src = nodesRef.current.find(n => n.id === nodeId);
        if (src) {
          const affected = (src.edges || [])
            .filter(e => e.edgeType === 'calls' || e.edgeType === 'data-flow')
            .map(e => e.targetId);
          if (affected.length) {
            setNodeExecStatus(s => {
              const next = { ...s };
              affected.forEach(id => { next[id] = 'warn'; });
              return next;
            });
          }
        }
    ...
    // Phase 6.3 — pipeline events
    sock.on('pipeline:start', ({ order }) => {
      setPipelineRunning(true);
      setPipelineLog([`▶ Running pipeline (${order.length} nodes)...`]);
    });
    sock.on('pipeline:node:start', ({ nodeId }) => {
      const n = nodesRef.current.find(x => x.id === nodeId);
      setPipelineLog(l => [...l, `  ↳ Running ${n?.label || nodeId}...`]);
      setNodeExecStatus(s => ({ ...s, [nodeId]: 'running' }));
    });
    sock.on('pipeline:node:done', ({ nodeId, exitCode }) => {
      setNodeExecStatus(s => ({ ...s, [nodeId]: exitCode === 0 ? 'success' : 'error' }));
      setPipelineLog(l => [...l, `    ${exitCode === 0 ? '✓' : '✗'} exit ${exitCode}`]);
    });
    sock.on('pipeline:done', ({ total }) => {
      setPipelineRunning(false);
      setPipelineLog(l => [...l, `✓ Pipeline complete (${total} nodes)`]);
    });
    sock.on('pipeline:error', ({ error }) => {
      setPipelineRunning(false);
      setPipelineLog(l => [...l, `✗ Pipeline failed: ${error}`]);
    });

    return () => sock.disconnect();
  }, [token, workspaceId]);

  // ── Feed + Templates loading ────────────────────────────────────────────
  useEffect(() => {
    if (!token || !workspaceId) return;
    apiFetch('/api/feed', {}, token).then(data => {
      if (Array.isArray(data)) { setFeedEntries(data); setFeedUnread(0); }
    }).catch(() => {});
  ...
  // ── Yjs CRDT + awareness ─────────────────────────────────────────────────
  useEffect(() => {
    if (!token || token === 'DEMO' || !workspaceId) return;
    if (!window.Y) { console.warn('Yjs not loaded, CRDT disabled'); return; }
    const Y = window.Y;
    const doc = new Y.Doc();
    ydocRef.current = doc;

    // Connect via y-websocket to our backend's CRDT namespace
    const wsUrl = API_URL.replace('http', 'ws') + '/crdt';
    const WsProvider = window.WebsocketProvider || (window.Y && window.Y.WebsocketProvider);
    if (!WsProvider) { console.warn('y-websocket not loaded, CRDT disabled'); return; }
    const wsProvider = new WsProvider(wsUrl, workspaceId, doc, {
      params: { token },
    });
    ywsRef.current = wsProvider;
    const awareness = wsProvider.awareness;
    awarenessRef.current = awareness;

    // Set our own presence state
    awareness.setLocalStateField('user', {
      id: operatorId, name: operatorName, color: AVATAR_ACCENTS[0],
      nodeId: null, // will update as we open files
    });
    ...
    const updatePeers = () => {
      ...
      awareness.getStates().forEach((state, clientId) => {
        ...
        if (state.user) states[clientId] = state.user;
      });
      setPeers(states);
    };
    ...
    return () => {
      ...
      wsProvider.destroy();
      doc.destroy();
      ydocRef.current = null; ywsRef.current = null; awarenessRef.current = null;
      setPeers({});
    };
  }, [token, workspaceId]);
  ...
  useEffect(() => {
    ...
    awarenessRef.current.setLocalStateField('user', {
      ...
      color: AVATAR_ACCENTS[avatarIndex % AVATAR_ACCENTS.length],
      nodeId: activeTabId || null,
    });
  }, [activeTabId]);

  // ── xterm.js: initialize when terminal panel opens ──────────────────────
  useEffect(() => {
    ...
    const xterm = new window.Terminal({
      ...
      fontSize: 12, lineHeight: 1.5, cursorBlink: true,
      theme: {
        background: termPalette.bg, foreground: termPalette.text,
        cursor: termPalette.cursor, selection: termPalette.selection,
        black: '#000000', red: termPalette.error, green: termPalette.prompt,
        yellow: termPalette.warn, blue: '#4285f4', magenta: '#bb9af7',
        cyan: termPalette.info, white: termPalette.text,
      },
    ...
    xterm.writeln('\x1b[32mFORBIDDEN TERMINAL\x1b[0m');
    if (token && workspaceId) {
      xterm.writeln('\x1b[2mConnecting to workspace container...\x1b[0m');
      // Open PTY session on backend
      socketRef.current?.emit('terminal:open', {
        workspaceId, sessionId: sessionIdRef.current,
        cols: xterm.cols, rows: xterm.rows,
      });
      setTermOpen(true);
      xterm.writeln('\x1b[2mType to interact with the container shell.\x1b[0m\r\n');
    } else {
      xterm.writeln('\x1b[2m[DEMO MODE] Backend not connected. Showing local simulation.\x1b[0m\r\n');
      xterm.write('$ ');
    }

    // Send input to backend PTY (or handle locally in demo mode)
    xterm.onData(data => {
      if (token && socketRef.current?.connected) {
        socketRef.current.emit('terminal:input', {
          workspaceId, sessionId: sessionIdRef.current, data,
        });
      } else {
        // Demo: local echo
        if (data === '\r') {
    ...
    if (xtermContainerRef.current) resizeObs.observe(xtermContainerRef.current);
  ...
  }, [bottomPanelTab]); // re-run when tab switches to terminal

  // Flush queued terminal output into xterm
  useEffect(() => {
    if (termOutputQueue.length === 0 || !xtermRef.current) return;
    termOutputQueue.forEach(chunk => xtermRef.current.write(chunk));
    setTermOutputQueue([]);
  }, [termOutputQueue]);

  const emitNodeEvent = useCallback((type, payload) => {
    if (!socketRef.current?.connected) return;
    const clientEventId = makeClientEventId();
    socketRef.current.emit('node:' + (type === 'NODE_CREATED' ? 'add' : type === 'NODE_EDITED' ? 'edit' : type === 'NODE_DELETED' ? 'delete' : type === 'NODE_JOINED' ? 'join' : 'cut'), {
      workspaceId, operatorId, sessionId: sessionIdRef.current,
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (!inInput) {
        ...
        if (e.key==='a'||e.key==='A') setSidebarMode(s=>s==='ai'?null:'ai');
        if (e.key==='l'||e.key==='L') runLayout();
        if (e.key==='Delete'||e.key==='Backspace') { if(hoveredNodeId){ nodesRef.current=nodesRef.current.filter(n=>n.id!==hoveredNodeId); edgesRef.current=edgesRef.current.filter(e=>e.source!==hoveredNodeId&&e.target!==hoveredNodeId); forceRender({}); emitNodeEvent('NODE_DELETED', { nodeId:hoveredNodeId }); } }
      }
    };
  ...
  // Phase 5.2 — backend search results
  const [searchResults, setSearchResults] = useState(null); // null = show all, [] = no results
  const searchDebounceRef = useRef(null);
  const handleSearch = (q) => {
    setSearchQuery(q);
    if (searchDebounceRef.current) clearTimeout(searchDebounceRef.current);
    if (!q.trim()) { setSearchResults(null); return; }
    if (!token) { setSearchResults(nodesRef.current.filter(n => n.label?.toLowerCase().includes(q.toLowerCase()) || (n.code||'').toLowerCase().includes(q.toLowerCase()))); return; }
    searchDebounceRef.current = setTimeout(async () => {
      try {
        const d = await apiFetch(`/api/workspaces/${workspaceId}/search?q=${encodeURIComponent(q)}`, {}, token);
        setSearchResults(d.results || []);
      } catch {
        setSearchResults(nodesRef.current.filter(n => n.label?.toLowerCase().includes(q.toLowerCase())));
      }
    }, 350);
  };

  const filteredSearch = searchResults !== null ? searchResults : nodesRef.current;
  const modifiedNodes = nodesRef.current.filter(n => n.modified);
  const visibleNodes = nodesRef.current; // All nodes visible; timeline shows event log not a playhead
  ...
  const wrapperClass = `app-wrapper theme-${themeMode} zone-${activeZoneKey}`;
  const outerBg = themeMode==='brutal' ? OUTSIDE_COLORS[activeZoneKey] : '#050505';
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Compute epoch from earliest node createdAt for stable Z reference
      const epoch = nodes.reduce((m,n)=>Math.min(m,n.createdAt||Date.now()),Date.now());

      // XY-only repulsion (Z is time — immutable)
      ...
      nodes.forEach(n => {
        ...
        n.vx*=0.8; n.vy*=0.8;
        n.x+=n.vx; n.y+=n.vy;
        // Z is always derived from createdAt — never drifts
        n.z = timeToZ(n.createdAt||Date.now(), epoch);
  ...
  const handleCreateNode = () => {
    const template = buildNodeTemplate(newNodeName, newNodeType);
    if (!template) return;
    ...
    setNewNodeColor(1);
    openNodeInEditor(nid);
    forceRender({});
    emitNodeEvent('NODE_CREATED', {
      nodeId:nid,
      label:newNode.label,
      type:newNodeType,
      code:newNode.code,
  ...
  const handleNodeClickInMode = (nodeId) => {
    if (edgeMode === 'join') {
      ...
      if (!exists) {
        edgesRef.current=[...edgesRef.current,{ id:'e'+Date.now(), source:joinFirstNode, target:nodeId, edgeType:joinEdgeType, label:'' }];
        forceRender({});
        emitNodeEvent('NODE_JOINED', { sourceId:joinFirstNode, targetId:nodeId, edgeType:joinEdgeType });
      }
  ...
  const handleEdgeClickInMode = (edgeId) => {
    if (edgeMode === 'cut') {
      ...
      forceRender({});
      if (edge) emitNodeEvent('NODE_CUT', { sourceId:edge.source, targetId:edge.target });
    }
  };
  ...
  const COLID_TO_STATUS = { c1:'archived', c2:'todo', c3:'in-progress', c4:'in-progress', c5:'done' };

  const addCard = (colId) => {
    if (!newCardTitle.trim()) return;
    const tempId = 'k' + Date.now();
    const newCard = { id: tempId, colId, title: newCardTitle.trim(), priority: 'MED', tags: [], progress: 0, due: '', assignee: avatarIndex };
    setBoard(b => ({ ...b, cards: [...b.cards, newCard] }));
    setNewCardCol(null); setNewCardTitle('');
    if (token && workspaceId) {
      apiFetch(`/api/tasks/${workspaceId}`, { method: 'POST', body: JSON.stringify({ title: newCard.title, status: COLID_TO_STATUS[colId] || 'todo' }) }, token)
        .then(data => {
          if (data?.task) {
            const bid = data.task._id || data.task.id;
            setBoard(b => ({ ...b, cards: b.cards.map(c => c.id === tempId ? { ...c, id: bid, _backendId: bid } : c) }));
          }
        }).catch(() => {});
    }
  };
  ...
  const moveCard = (cardId, colId) => {
    ...
    const bid = card?._backendId || cardId;
    if (token && workspaceId && bid && !/^k\d/.test(bid)) {
      apiFetch(`/api/tasks/${workspaceId}/${bid}`, { method: 'PATCH', body: JSON.stringify({ status: COLID_TO_STATUS[colId] || 'todo' }) }, token).catch(() => {});
    }
  };
  ...
  const deleteCard = (cardId) => {
    ...
    const bid = card?._backendId || cardId;
    if (token && workspaceId && bid && !/^k\d/.test(bid)) {
      apiFetch(`/api/tasks/${workspaceId}/${bid}`, { method: 'DELETE' }, token).catch(() => {});
    }
  };

  // Phase 4 — run a node
  const runNode = (nodeId) => {
    setNodeOutputs(o => ({ ...o, [nodeId]: '' }));
    setNodeExecStatus(s => ({ ...s, [nodeId]: 'running' }));
    socketRef.current?.emit('node:run', { nodeId });
  };

  // Phase 5.1 — timeline scrubber: load historical events then reconstruct state
  const handleScrub = useCallback(async (pos) => {
    setScrubberPos(pos);
    if (pos >= 100) {
      // Return to live
      setReplayMode(false);
    ...
    setReplayMode(true);
    if (!token || !workspaceId) return;
    // Map 0–99 position to an event index in allTimelineEvents
    const events = allTimelineEvents;
    if (events.length === 0) return;
    const idx = Math.floor((pos / 100) * (events.length - 1));
    const cutoff = events[idx]?.createdAt;
    if (!cutoff) return;
    // Fetch events up to this timestamp from the backend
    try {
      const data = await apiFetch(`/api/events?workspaceId=${workspaceId}&limit=500`, {}, token);
      const replayEvents = (data.events || []).filter(e => e.meta?.serverTimestamp <= cutoff);
      // Reconstruct graph from replayed events
      const rNodes = {};
      const rEdges = {};
      replayEvents.forEach(ev => {
        const p = ev.payload || {};
        if (ev.type === 'NODE_CREATED') {
          rNodes[p.nodeId] = { id: p.nodeId, label: p.label || 'Untitled', type: p.type || 'function', isMain: false, x: p.position?.x || 0, y: p.position?.y || 0, vx: 0, vy: 0, themeIdx: 0, classId: null, code: p.code || '', modified: false };
        } else if (ev.type === 'NODE_EDITED' && rNodes[p.nodeId]) {
          rNodes[p.nodeId] = { ...rNodes[p.nodeId], code: p.code ?? rNodes[p.nodeId].code };
        } else if (ev.type === 'NODE_DELETED') {
          delete rNodes[p.nodeId];
        } else if (ev.type === 'NODE_JOINED') {
          const eid = `${p.sourceId}-${p.targetId}`;
          rEdges[eid] = { id: eid, source: p.sourceId, target: p.targetId, edgeType: p.edgeType || 'default', label: p.label || '' };
        } else if (ev.type === 'NODE_CUT') {
          delete rEdges[`${p.sourceId}-${p.targetId}`];
        }
      });
      nodesRef.current = Object.values(rNodes);
      edgesRef.current = Object.values(rEdges);
      forceRender({});
    } catch (e) {
      console.warn('[scrubber] replay failed:', e.message);
    }
  }, [token, workspaceId, allTimelineEvents]);

  // Phase 5 — create share link
  const handleShare = async () => {
    if (!token || !workspaceId) return;
    setShareLoading(true);
    try {
      const d = await apiFetch(`/api/workspaces/${workspaceId}/share`, { method: 'POST' }, token);
      setShareLink(d.url);
    } catch (e) {
      setShareLink('error');
  ...
  // Phase 6.3 — run pipeline
  const runPipeline = () => {
    const nodeIds = nodesRef.current.map(n => n.id);
    socketRef.current?.emit('pipeline:run', { nodeIds });
  };

  // Phase 3.2 — auto-layout with Dagre
  const runLayout = useCallback(() => {
    if (!window.dagre) { console.warn('[layout] dagre not loaded'); return; }
    const g = new window.dagre.graphlib.Graph();
    g.setGraph({ rankdir: 'LR', nodesep: 80, ranksep: 140, marginx: 60, marginy: 60 });
    g.setDefaultEdgeLabel(() => ({}));

    const unpinned = nodesRef.current.filter(n => !n.pinned);
    unpinned.forEach(n => g.setNode(n.id, { width: 60, height: 60 }));
    ...
    // Apply positions — centre the layout around origin
    const laidOut = unpinned.map(n => {
      const p = g.node(n.id);
      return p ? { id: n.id, x: p.x - (g.graph().width || 0) / 2, y: p.y - (g.graph().height || 0) / 2 } : null;
    }).filter(Boolean);

    nodesRef.current = nodesRef.current.map(n => {
      const p = laidOut.find(l => l.id === n.id);
      return p ? { ...n, x: p.x, y: p.y, vx: 0, vy: 0 } : n;
    });
    forceRender({});

    laidOut.forEach(({ id, x, y }) => {
      const n = nodesRef.current.find(nn=>nn.id===id);
      if (n) n.z = 0; // flatten to XY plane after layout
      socketRef.current?.emit('node:edit', {
        workspaceId, nodeId: id, position: { x, y },
        operatorId, sessionId: sessionIdRef.current,
        ...(makeClientEventId() ? { clientEventId: makeClientEventId() } : {}),
      });
    });
  }, [workspaceId, operatorId]);
  ...
  const sendAiMessage = async () => {
    ...
    setAiLoading(true);
    try {
      // Build topology-aware context
      const upstream = activeNode ? (activeNode.edges || []).slice(0,3).map(e => {
        const n = nodesRef.current.find(x => x.id === e.targetId);
        return n ? `${n.label} (${n.language}):\n${(n.code||'').slice(0,300)}` : '';
      }).filter(Boolean) : [];
      const systemCtx = activeNode
        ? `You are an AI assistant embedded in the FORBIDDEN graph IDE. Current node: ${activeNode.label} (${activeNode.language}). Mode: ${aiMode}.\nCurrent code:\n\`\`\`\n${(activeNode.code||'').slice(0,1500)}\n\`\`\`\n${upstream.length ? `\nUpstream nodes:\n${upstream.join('\n---\n')}` : ''}`
        : `You are an AI assistant in the FORBIDDEN graph IDE. Mode: ${aiMode}. No file currently open.`;

      const resp = await fetch(`${API_URL.replace(/\/$/, '')}/api/ai/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ messages: [...aiMessages, { role: 'user', content: userMsg }], system: systemCtx }),
      });
      const d = await resp.json();
      const reply = d.reply || d.content?.[0]?.text || '(no response)';
  ...
  const sendChatMessage = useCallback(() => {
    ...
    const msg = { id:Date.now(), from: operatorName || 'You', time: new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}), text, self:true };
    setChatMessages(m => [...m, msg]);
    setChatInput('');
    if (token && workspaceId) {
      apiFetch(`/api/messages/${workspaceId}`, { method:'POST', body:JSON.stringify({ text, operatorId, operatorName }) }, token).catch(() => {});
    }
  }, [chatInput, token, workspaceId, operatorId, operatorName]);
  ...
  const handleCmdAction = (label) => {
    ...
    else if(label.includes('dark / brutal')) setThemeMode(m=>m==='cyber'?'brutal':'cyber');
  ...
  return (
    <div style={{width:'100vw',height:'100vh',padding:themeMode==='brutal'?'36px':'0px',backgroundColor:outerBg,transition:'all 0.4s ease',boxSizing:'border-box'}}>
      <div className={wrapperClass}>
        ...
        <div className="app-header">
          ...
          <div style={{display:'flex',alignItems:'center',gap:'20px'}}>
            ...
            <div className="hdr-breadcrumb">
              <span style={{opacity:0.3}}>{workspaceName || 'workspace'}</span>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'5px',marginLeft:'12px'}}>
              <span style={{width:'5px',height:'5px',borderRadius:'50%',display:'inline-block',transition:'background 0.4s',background:wsStatus==='live'?'#10b981':wsStatus==='connecting'?'#ffc410':'#ff435a',boxShadow:wsStatus==='live'?'0 0 6px #10b981':wsStatus==='connecting'?'0 0 6px #ffc410':'0 0 6px #ff435a'}}/>
              <span style={{fontSize:'8px',opacity:0.3,letterSpacing:'1px'}}>{wsStatus.toUpperCase()}</span>
            </div>
            {/* Phase 1.3 — Container status dot */}
            <div style={{display:'flex',alignItems:'center',gap:'5px'}} title={`Container: ${containerStatus}`}>
              <span style={{width:'5px',height:'5px',borderRadius:'50%',display:'inline-block',background:containerStatus==='running'?'#10b981':containerStatus==='starting'?'#ffc410':containerStatus==='paused'?'#888':'#ff435a',boxShadow:containerStatus==='running'?'0 0 6px #10b981':containerStatus==='starting'?'0 0 6px #ffc410 0.4s infinite':''}}/>
              <span style={{fontSize:'8px',opacity:0.25,letterSpacing:'1px'}}>CTR</span>
              {(containerStatus === 'paused' || containerStatus === 'dead') && token && (
                <button onClick={async()=>{setContainerStatus('starting');await apiFetch(`/api/workspaces/${workspaceId}/wake`,{method:'POST'},token).catch(()=>{});}} style={{fontSize:'8px',padding:'1px 5px',border:'1px solid rgba(255,200,0,0.3)',background:'transparent',color:'#ffc410',cursor:'pointer',borderRadius:'2px',letterSpacing:'1px'}}>WAKE</button>
              )}
            </div>
          </div>
          ...
          <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
            ...
            {/* Phase 6.3 — Pipeline run */}
            <button className="btn hdr-pill" style={{display:'flex',alignItems:'center',gap:'5px',color:pipelineRunning?'#ffc410':'inherit'}} onClick={runPipeline} disabled={pipelineRunning} title="Run pipeline (all nodes in dependency order)">
            ...
            {/* Phase 5 — Share link */}
            <button className="btn hdr-pill" style={{display:'flex',alignItems:'center',gap:'5px'}} onClick={handleShare} disabled={shareLoading} title="Create read-only share link">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
              <span>{shareLoading?'..':'SHARE'}</span>
            </button>
            {/* Templates */}
            <button className="btn hdr-pill" style={{display:'flex',alignItems:'center',gap:'5px'}} onClick={()=>setShowTemplates(v=>!v)} title="Browse workspace templates">
            ...
            {Object.keys(peers).length > 0 && (
              <div style={{display:'flex',alignItems:'center',gap:'4px',padding:'4px 8px',border:'1px solid rgba(16,185,129,0.2)',borderRadius:'20px',background:'rgba(16,185,129,0.06)'}}>
                {Object.values(peers).slice(0,3).map((p,i)=>(
                  <div key={i} title={p.name} style={{width:'16px',height:'16px',borderRadius:'50%',background:p.color||'#10b981',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'8px',color:'#fff',fontWeight:'bold',marginLeft:i>0?'-4px':'0',border:'1.5px solid rgba(8,8,16,0.8)'}}>{(p.name||'?')[0].toUpperCase()}</div>
                ))}
                <span style={{fontSize:'9px',color:'#10b981',opacity:0.7,marginLeft:'2px'}}>{Object.keys(peers).length} ONLINE</span>
              </div>
            )}
            {/* Theme toggle */}
            ...
            {/* Phase 6 — AI panel toggle */}
            <button className="btn hdr-pill" style={{display:'flex',alignItems:'center',gap:'5px',color:sidebarMode==='ai'?'#10b981':'inherit',borderColor:sidebarMode==='ai'?'rgba(16,185,129,0.4)':'undefined'}} onClick={()=>setSidebarMode(s=>s==='ai'?null:'ai')} title="AI Assistant (A)">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2a10 10 0 1 0 10 10H12z"/><path d="M12 12l4-4"/><circle cx="12" cy="12" r="1"/></svg>
              <span>AI</span>
            </button>
            {/* Logout */}
            {onLogout && (
              <button className="btn hdr-pill" style={{opacity:0.4,fontSize:'9px'}} onClick={onLogout} title="Logout">⏻</button>
            )}
          </div>
        </div>
        ...
        <div className="app-body">
          ...
          <div className="sidebar">
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'2px',width:'100%',alignItems:'center',paddingTop:'6px'}}>
              {[
                ...
                ['note',    'NOTES',            <I.Note/>],
                ['board',   'BOARD',            <I.Board/>],
                ['feed',    'ACTIVITY FEED',   <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>{feedUnread>0&&<div className="badge">{feedUnread>9?'9+':feedUnread}</div>}</>],
                ['ai',      'AI ASSISTANT',    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/><circle cx="12" cy="16" r="1"/></svg>],
              ].map(([mode,tip,icon])=>(
            ...
            <div className={`strip-icon ${sidebarMode==='settings'?'active':''}`} onClick={()=>setSidebarMode(s=>s==='settings'?null:'settings')}
              ...
              <CyberAvatar index={avatarIndex} size={26}/><div className="strip-icon-tooltip">WORKSPACE</div>
            </div>
          </div>
          ...
          <FloatingPanel title="SEARCH" isOpen={sidebarMode==='search'} onClose={()=>setSidebarMode(null)} defaultX={70} defaultY={70} defaultW={290} defaultH={420}>
            <div style={{padding:'10px 12px',borderBottom:'1px solid rgba(128,128,128,0.1)'}}>
              <input className="side-input" value={searchQuery} onChange={e=>handleSearch(e.target.value)} placeholder="Search labels and code..." autoFocus={sidebarMode==='search'}/>
            </div>
          ...
          <FloatingPanel title="SOURCE CONTROL" isOpen={sidebarMode==='git'} onClose={()=>setSidebarMode(null)} defaultX={70} defaultY={70} defaultW={270} defaultH={400}>
            <div style={{padding:'8px 12px',borderBottom:'1px solid rgba(128,128,128,0.1)',fontSize:'9px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{opacity:0.5}}>main ← origin/main</span>
              <span style={{color:modifiedNodes.length?'#ffc410':'#10b981'}}>{modifiedNodes.length} unsaved</span>
            </div>
            <div style={{flex:1,overflowY:'auto'}}>
              {modifiedNodes.length===0
                ? <div style={{padding:'20px',opacity:0.3,fontSize:'10px',textAlign:'center'}}>Working tree clean.</div>
                : modifiedNodes.map(n => (
                  <div key={'g_'+n.id} className="list-item" style={{display:'flex',alignItems:'center',gap:'8px',padding:'8px 12px'}} onClick={()=>openNodeInEditor(n.id)}>
                    <I.FileIcon/>
                    <span style={{color:'#ffc410',flex:1,fontSize:'10px'}}>{n.label}</span>
                    <span style={{fontSize:'8px',color:'#ffc410',opacity:0.7,fontFamily:"'JetBrains Mono',monospace"}}>M</span>
                    <button style={{fontSize:'8px',padding:'2px 7px',background:'rgba(16,185,129,0.1)',border:'1px solid rgba(16,185,129,0.3)',borderRadius:'3px',color:'#10b981',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace"}}
                      onClick={e=>{e.stopPropagation();const node=nodesRef.current.find(nd=>nd.id===n.id);if(node){node.modified=false;forceRender({});emitNodeEvent('NODE_EDITED',{nodeId:n.id,code:node.code,language:detectLanguage(node.label),saved:true});}}}
                    >SAVE</button>
                  </div>
                ))}
            </div>
            {modifiedNodes.length > 0 && (
              <div style={{padding:'10px 12px',borderTop:'1px solid rgba(128,128,128,0.08)',flexShrink:0}}>
                <button className="btn" style={{width:'100%',fontSize:'9px',color:'#10b981',borderColor:'rgba(16,185,129,0.3)',background:'rgba(16,185,129,0.05)'}}
                  onClick={()=>{nodesRef.current.forEach(n=>{if(n.modified){n.modified=false;emitNodeEvent('NODE_EDITED',{nodeId:n.id,code:n.code,language:detectLanguage(n.label),saved:true});}});forceRender({});}}>
                  ✓ SAVE ALL {modifiedNodes.length} FILES
                </button>
              </div>
            )}
          </FloatingPanel>

          <FloatingPanel title="WORKSPACE" isOpen={sidebarMode==='settings'} onClose={()=>setSidebarMode(null)} defaultX={70} defaultY={Math.max(50,window.innerHeight-500)} defaultW={460} defaultH={460}>
            <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
              {/* Operator identity */}
              <div style={{padding:'16px 20px',borderBottom:'1px solid rgba(128,128,128,0.1)'}}>
                <div style={{fontSize:'9px',opacity:0.4,marginBottom:'10px',letterSpacing:'1.2px'}}>OPERATOR IDENTITY</div>
                <div style={{display:'flex',alignItems:'center',gap:'14px'}}>
                  <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                    {[0,1,2,3,4,5].map(i=>(<CyberAvatar key={i} index={i} size={40} selected={avatarIndex===i} onClick={()=>setAvatarIndex(i)}/>))}
                  </div>
                  <div>
                    <div style={{fontWeight:'bold',fontSize:'12px',letterSpacing:'1px'}}>{operatorName||'OPERATOR'}</div>
                    <div style={{fontSize:'9px',opacity:0.3,marginTop:'2px',fontFamily:"'JetBrains Mono',monospace"}}>{operatorId}</div>
                  </div>
                </div>
              </div>
              {/* Workspace info */}
              <div style={{padding:'16px 20px',borderBottom:'1px solid rgba(128,128,128,0.1)'}}>
                <div style={{fontSize:'9px',opacity:0.4,marginBottom:'10px',letterSpacing:'1.2px'}}>CURRENT WORKSPACE</div>
                <div style={{display:'flex',gap:'16px',fontSize:'10px'}}>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:'bold',fontSize:'13px',marginBottom:'4px'}}>{workspaceName||'—'}</div>
                    <div style={{opacity:0.3,fontFamily:"'JetBrains Mono',monospace",fontSize:'9px',wordBreak:'break-all'}}>{workspaceId}</div>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',gap:'6px',fontSize:'9px',opacity:0.5,textAlign:'right',flexShrink:0}}>
                    <span>{nodesRef.current.length} nodes</span>
                    <span>{edgesRef.current.length} edges</span>
                    <span>{groupsRef.current.length} groups</span>
                  </div>
                </div>
              </div>
              {/* Live presence */}
              <div style={{padding:'16px 20px',borderBottom:'1px solid rgba(128,128,128,0.1)',flex:1,overflowY:'auto'}}>
                <div style={{fontSize:'9px',opacity:0.4,marginBottom:'10px',letterSpacing:'1.2px',display:'flex',justifyContent:'space-between'}}>
                  <span>ONLINE OPERATORS</span>
                  <span style={{color:'#10b981'}}>{Object.keys(peers).length + 1} ONLINE</span>
                </div>
                {/* Self */}
                <div style={{display:'flex',alignItems:'center',gap:'10px',padding:'6px 0',opacity:0.7}}>
                  <CyberAvatar index={avatarIndex} size={28}/>
                  <div>
                    <div style={{fontSize:'10px',fontWeight:'bold'}}>{operatorName||'You'} <span style={{color:'#10b981',fontSize:'8px',marginLeft:'4px'}}>YOU</span></div>
                    <div style={{fontSize:'9px',opacity:0.4}}>{activeTabId ? (nodesRef.current.find(n=>n.id===activeTabId)?.label||'editing') : 'browsing'}</div>
                  </div>
                </div>
                {/* Peers */}
                {Object.values(peers).map((peer, i) => (
                  <div key={i} style={{display:'flex',alignItems:'center',gap:'10px',padding:'6px 0',borderTop:'1px solid rgba(255,255,255,0.04)'}}>
                    <div style={{width:'28px',height:'28px',borderRadius:'50%',background:peer.color||'#4285f4',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',color:'#fff',fontWeight:'bold',flexShrink:0}}>{(peer.name||'?')[0].toUpperCase()}</div>
                    <div>
                      <div style={{fontSize:'10px',fontWeight:'bold'}}>{peer.name||'Operator'}</div>
                      <div style={{fontSize:'9px',opacity:0.4}}>{peer.nodeId ? (nodesRef.current.find(n=>n.id===peer.nodeId)?.label||'editing') : 'browsing'}</div>
                    </div>
                    <div style={{marginLeft:'auto',width:'7px',height:'7px',borderRadius:'50%',background:'#10b981',boxShadow:'0 0 6px #10b981'}}/>
                  </div>
                ))}
                {Object.keys(peers).length === 0 && (
                  <div style={{fontSize:'9px',opacity:0.2,paddingTop:'8px'}}>No other operators online</div>
                )}
              </div>
              {/* Actions */}
              <div style={{padding:'12px 20px',display:'flex',gap:'8px',flexShrink:0,borderTop:'1px solid rgba(128,128,128,0.08)'}}>
                <button className="btn" style={{fontSize:'9px',flex:1}} onClick={()=>{setSidebarMode(null);setShowTemplates(true);}}>⬡ TEMPLATES</button>
                <button className="btn" style={{fontSize:'9px',flex:1}} onClick={()=>setSidebarMode('feed')}>◈ ACTIVITY</button>
                <button className="btn" style={{fontSize:'9px',color:'#ff435a',borderColor:'rgba(255,67,90,0.3)'}} onClick={()=>{if(confirm('Leave workspace?'))window.location.reload();}}>← LEAVE</button>
              </div>
            </div>
          </FloatingPanel>
          ...
          <FloatingPanel title="COMMUNICATIONS" isOpen={sidebarMode==='chat'} onClose={()=>setSidebarMode(null)} defaultX={360} defaultY={70} defaultW={300} defaultH={400}>
            <div className="chat-wrap">
              <div style={{padding:'6px 12px',borderBottom:'1px solid rgba(128,128,128,0.1)',fontSize:'9px',opacity:0.4,display:'flex',gap:'10px',alignItems:'center'}}>
                <span style={{color:'#10b981',display:'flex',alignItems:'center',gap:'4px'}}>
                  <div style={{width:'5px',height:'5px',borderRadius:'50%',background:'#10b981'}}/>
                  {Object.keys(peers).length + 1} online
                </span>
                <span style={{marginLeft:'auto'}}>#{workspaceName||'general'}</span>
              </div>
              ...
              <div className="chat-input-wrap">
                ...
                <button className="btn" style={{fontSize:'9px',flexShrink:0}} onClick={sendChatMessage}>SEND</button>
              </div>
            </div>
          </FloatingPanel>
            ...
            <div style={{flex:1,overflowY:'auto',padding:'10px 12px',display:'flex',flexDirection:'column',gap:'12px'}}>
              ...
              <div style={{padding:'10px 12px',background:'rgba(128,128,128,0.04)',border:'1px solid rgba(128,128,128,0.1)',borderRadius:'4px'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                  <div style={{width:'10px',height:'10px',borderRadius:'50%',background:AVATAR_ACCENTS[activeTabNode?.themeIdx%AVATAR_ACCENTS.length||0],boxShadow:`0 0 8px ${AVATAR_ACCENTS[activeTabNode?.themeIdx%AVATAR_ACCENTS.length||0]}`}}/>
              ...
              return (
                <div className="board-wrap" style={{position:'relative'}}>
                  <div className="board-topbar">
                    <span style={{fontSize:'11px',fontWeight:'bold',opacity:0.8,letterSpacing:'0.8px'}}>FORBIDDEN / SPRINT-01</span>
          ...
          <FloatingPanel
            ...
            {bottomPanelTab==='terminal' && (
              <div style={{display:'flex',flexDirection:'column',flex:1,overflow:'hidden',background:termPalette.bg,transition:'background 0.3s'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:'6px',padding:'5px 10px',borderBottom:`1px solid ${termPalette.border}`,background:termPalette.bg,flexShrink:0,minHeight:'32px'}}>
                  ...
                  {token && workspaceId && (
                    <div style={{display:'flex',alignItems:'center',gap:'4px',marginLeft:'8px'}}>
                      <span style={{width:'5px',height:'5px',borderRadius:'50%',display:'inline-block',background:termOpen?'#10b981':'#ff435a'}}/>
                      <span style={{fontSize:'8px',opacity:0.35,letterSpacing:'1px'}}>{termOpen?'PTY LIVE':'DISCONNECTED'}</span>
                    </div>
                  )}
                  <div style={{marginLeft:'auto',display:'flex',gap:'5px',alignItems:'center',position:'relative'}}>
                    <button onClick={()=>{ xtermRef.current?.clear(); }}
                    ...
                    {showTermPalette && (
                      <div style={{position:'absolute',top:'calc(100% + 6px)',right:0,zIndex:9999,background:'#0a0a0e',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'8px',padding:'8px',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'4px',minWidth:'300px',boxShadow:'0 16px 50px rgba(0,0,0,0.9)',maxHeight:'320px',overflowY:'auto'}}
                        ...
                        {TERM_PALETTES.map(tp => (
                          <div key={tp.id}
                            onClick={()=>{setTermPalette(tp);setShowTermPalette(false);if(xtermRef.current){xtermRef.current.options.theme={background:tp.bg,foreground:tp.text,cursor:tp.cursor,selection:tp.selection};}}}
                ...
                <div ref={xtermContainerRef} style={{flex:1,minHeight:0,overflow:'hidden',padding:'4px'}}
                  onClick={()=>{ setShowTermPalette(false); xtermRef.current?.focus(); }}/>
              </div>
            )}
            {bottomPanelTab==='timeline' && (
              <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
                {/* ── Scrubber bar ── */}
                <div style={{padding:'8px 14px 6px',borderBottom:'1px solid rgba(255,255,255,0.05)',flexShrink:0,background:'rgba(0,0,0,0.2)'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'6px'}}>
                    <span style={{fontSize:'8px',opacity:0.35,letterSpacing:'2px',whiteSpace:'nowrap',fontFamily:"'JetBrains Mono',monospace"}}>HISTORY</span>
                    <div style={{flex:1,position:'relative',height:'20px',display:'flex',alignItems:'center'}}>
                      {/* tick marks */}
                      <div style={{position:'absolute',inset:'0 0 0 0',display:'flex',alignItems:'center',pointerEvents:'none'}}>
                        {allTimelineEvents.slice(0, 40).map((ev, i) => (
                          <div key={i} style={{position:'absolute',left:`${(i / Math.max(allTimelineEvents.length-1,1))*100}%`,width:'2px',height:'8px',background:ev.accentColor||'rgba(255,255,255,0.15)',borderRadius:'1px',transform:'translateX(-50%)'}}/>
                        ))}
                      </div>
                      <input type="range" min="0" max="100" step="1" value={scrubberPos}
                        onChange={e => handleScrub(Number(e.target.value))}
                        style={{width:'100%',appearance:'none',height:'3px',background:`linear-gradient(to right, rgba(16,185,129,0.6) ${scrubberPos}%, rgba(255,255,255,0.1) ${scrubberPos}%)`,borderRadius:'2px',outline:'none',cursor:'pointer',position:'relative',zIndex:1}}/>
                    </div>
                    {replayMode ? (
                      <button onClick={()=>handleScrub(100)} style={{padding:'3px 10px',background:'rgba(255,67,90,0.12)',border:'1px solid rgba(255,67,90,0.3)',color:'#ff435a',borderRadius:'4px',fontSize:'8px',cursor:'pointer',fontFamily:'inherit',letterSpacing:'1px',whiteSpace:'nowrap',flexShrink:0}}>
                        ↩ LIVE
                      </button>
                    ) : (
                      <span style={{fontSize:'8px',opacity:0.3,letterSpacing:'1px',whiteSpace:'nowrap',flexShrink:0}}>LIVE ●</span>
                    )}
                  </div>
                  {replayMode && (
                    <div style={{fontSize:'8px',color:'#ff435a',opacity:0.75,letterSpacing:'1px',textAlign:'center',padding:'2px 0',background:'rgba(255,67,90,0.07)',borderRadius:'3px'}}>
                      ◀ HISTORY — READ ONLY — drag slider right to return to live
                    </div>
                  )}
                </div>
                {/* ── Event log ── */}
                <div style={{flex:1,overflowY:'auto',padding:'4px 0'}}>
                  {timelineEntries.length === 0 ? (
                    <div style={{textAlign:'center',padding:'32px',opacity:0.2,fontSize:'11px',letterSpacing:'2px',fontFamily:"'JetBrains Mono',monospace"}}>
                      {token ? 'NO EVENTS YET' : 'CONNECT TO BACKEND TO SEE TIMELINE'}
                    </div>
                  ) : timelineEntries.map((entry, i) => (
                    <div key={entry.eventId || i} style={{display:'flex',alignItems:'center',gap:'10px',padding:'6px 14px',borderBottom:'1px solid rgba(128,128,128,0.06)',cursor:'default',opacity:replayMode&&i===0?1:replayMode?0.5:1,transition:'opacity 0.2s'}}
                      onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'}
                      onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      <span style={{fontSize:'14px',minWidth:'18px',textAlign:'center'}}>{entry.icon || '◈'}</span>
                      <span style={{fontSize:'10px',flex:1,opacity:0.7}}>{entry.label}</span>
                      <span style={{fontSize:'9px',opacity:0.3,fontFamily:"'JetBrains Mono',monospace",whiteSpace:'nowrap'}}>
                        {entry.createdAt ? new Date(entry.createdAt).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}) : ''}
                      </span>
                      {entry.accentColor && <span style={{width:'4px',height:'4px',borderRadius:'50%',background:entry.accentColor,flexShrink:0}}/>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {bottomPanelTab==='audit' && (
              <AuditPanel token={token} workspaceId={workspaceId}/>
            )}
          </FloatingPanel>

          {/* ── ACTIVITY FEED ── */}
          <FloatingPanel title={<div style={{display:'flex',alignItems:'center',gap:'8px'}}>ACTIVITY FEED{feedUnread>0&&<span style={{background:'#ff435a',color:'#fff',borderRadius:'10px',padding:'1px 6px',fontSize:'8px',fontWeight:'bold'}}>{feedUnread}</span>}</div>} isOpen={sidebarMode==='feed'} onClose={()=>{setSidebarMode(null);setFeedUnread(0);apiFetch('/api/feed/read-all',{method:'POST'},token).catch(()=>{});}} defaultX={70} defaultY={70} defaultW={300} defaultH={500}>
            <div style={{flex:1,overflowY:'auto'}}>
              {feedEntries.length === 0 ? (
                <div style={{textAlign:'center',padding:'40px 20px',opacity:0.2,fontSize:'10px',letterSpacing:'1px'}}>
                  {token ? 'No activity yet' : 'Connect to see activity'}
                </div>
              ) : feedEntries.map((entry, i) => (
                <div key={entry.eventId||i} className="list-item" style={{padding:'10px 14px',borderBottom:'1px solid rgba(128,128,128,0.08)',display:'flex',gap:'10px',alignItems:'flex-start'}}>
                  <span style={{fontSize:'15px',flexShrink:0,marginTop:'1px'}}>{entry.icon||'◈'}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:'10px',opacity:0.75,lineHeight:'1.5',wordBreak:'break-word'}}>{entry.label||entry.type}</div>
                    <div style={{fontSize:'9px',opacity:0.3,marginTop:'3px',fontFamily:"'JetBrains Mono',monospace"}}>{entry.createdAt?new Date(entry.createdAt).toLocaleString('en-US',{hour:'2-digit',minute:'2-digit',month:'short',day:'numeric'}):''}</div>
                  </div>
                  {entry.accentColor&&<div style={{width:'3px',borderRadius:'2px',background:entry.accentColor,alignSelf:'stretch',flexShrink:0}}/>}
                </div>
              ))}
            </div>
          </FloatingPanel>
          ...
          {/* ── PHASE 6: AI ASSISTANT ── */}
          <FloatingPanel title={<div style={{display:'flex',alignItems:'center',gap:'8px'}}>
            ...
            AI ASSISTANT
          </div>} isOpen={sidebarMode==='ai'} onClose={()=>setSidebarMode(null)} defaultX={70} defaultY={70} defaultW={360} defaultH={540}>
            {/* Mode selector */}
            <div style={{display:'flex',gap:'4px',padding:'8px 12px',borderBottom:'1px solid rgba(128,128,128,0.08)'}}>
              {['explain','debug','refactor'].map(m=>(
                <button key={m} onClick={()=>setAiMode(m)} style={{flex:1,padding:'4px 0',fontSize:'9px',letterSpacing:'1px',border:`1px solid ${aiMode===m?'rgba(16,185,129,0.5)':'rgba(255,255,255,0.08)'}`,background:aiMode===m?'rgba(16,185,129,0.1)':'transparent',color:aiMode===m?'#10b981':'inherit',cursor:'pointer',borderRadius:'3px',fontFamily:"inherit",textTransform:'uppercase'}}>{m}</button>
              ))}
            </div>
            {/* Context indicator */}
            {activeTabId && <div style={{padding:'6px 12px',fontSize:'9px',opacity:0.35,borderBottom:'1px solid rgba(128,128,128,0.05)'}}> Context: {nodesRef.current.find(n=>n.id===activeTabId)?.label || 'current node'}</div>}
            {/* Messages */}
            <div style={{flex:1,overflowY:'auto',padding:'8px 12px',display:'flex',flexDirection:'column',gap:'10px'}}>
              {aiMessages.length === 0 && (
                <div style={{textAlign:'center',padding:'30px 10px',opacity:0.2,fontSize:'10px',lineHeight:'1.7'}}>
                  Ask me to explain, debug, or refactor your current node's code.<br/>
                  I can see upstream nodes too.
                </div>
              )}
              {aiMessages.map((m, i) => (
                <div key={i} style={{display:'flex',flexDirection:'column',gap:'3px',alignItems:m.role==='user'?'flex-end':'flex-start'}}>
                  <div style={{fontSize:'8px',opacity:0.3,letterSpacing:'1px'}}>{m.role==='user'?'YOU':'AI'}</div>
                  <div style={{maxWidth:'90%',padding:'8px 11px',borderRadius:'6px',fontSize:'10px',lineHeight:'1.6',whiteSpace:'pre-wrap',wordBreak:'break-word',background:m.role==='user'?'rgba(16,185,129,0.12)':'rgba(255,255,255,0.04)',border:`1px solid ${m.role==='user'?'rgba(16,185,129,0.2)':'rgba(255,255,255,0.06)'}`}}>{m.content}</div>
                </div>
              ))}
              {aiLoading && <div style={{fontSize:'10px',opacity:0.4,padding:'4px 0'}}>▋ thinking...</div>}
            </div>
            {/* Input */}
            <div style={{padding:'8px 12px',borderTop:'1px solid rgba(128,128,128,0.08)',display:'flex',gap:'6px'}}>
              <input value={aiInput} onChange={e=>setAiInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&(e.preventDefault(),sendAiMessage())}
                placeholder={`${aiMode.charAt(0).toUpperCase()+aiMode.slice(1)} this code...`}
                style={{flex:1,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',color:'inherit',padding:'7px 10px',fontSize:'10px',borderRadius:'4px',outline:'none',fontFamily:'inherit'}}/>
              <button onClick={sendAiMessage} disabled={aiLoading||!aiInput.trim()} style={{padding:'7px 12px',background:'rgba(16,185,129,0.15)',border:'1px solid rgba(16,185,129,0.3)',color:'#10b981',cursor:'pointer',borderRadius:'4px',fontSize:'10px',fontFamily:'inherit'}}>▶</button>
            </div>
          </FloatingPanel>

          {/* ── PHASE 5: SHARE LINK MODAL ── */}
          {shareLink && (
            <div style={{position:'fixed',top:0,left:0,width:'100%',height:'100%',background:'rgba(0,0,0,0.7)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setShareLink(null)}>
              <div style={{background:'#0d0d1a',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'10px',padding:'28px',width:'420px',maxWidth:'90vw'}} onClick={e=>e.stopPropagation()}>
                <div style={{fontSize:'10px',opacity:0.35,letterSpacing:'2px',marginBottom:'14px'}}>SHARE LINK CREATED</div>
                {shareLink === 'error' ? (
                  <div style={{color:'#ff435a',fontSize:'10px'}}>Failed to create share link. Connect to backend first.</div>
                ) : (
                  <>
                    <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'5px',padding:'10px 12px',fontSize:'10px',fontFamily:"'JetBrains Mono',monospace",wordBreak:'break-all',color:'#10b981',marginBottom:'14px'}}>{shareLink}</div>
                    <div style={{fontSize:'9px',opacity:0.3,marginBottom:'14px'}}>Read-only snapshot. Expires in 24 hours.</div>
                    <div style={{display:'flex',gap:'8px'}}>
                      <button onClick={()=>{navigator.clipboard.writeText(shareLink).catch(()=>{});}} style={{flex:1,padding:'8px',background:'rgba(16,185,129,0.1)',border:'1px solid rgba(16,185,129,0.3)',color:'#10b981',cursor:'pointer',borderRadius:'4px',fontSize:'10px',fontFamily:'inherit'}}>COPY LINK</button>
                      <button onClick={()=>setShareLink(null)} style={{padding:'8px 14px',background:'transparent',border:'1px solid rgba(255,255,255,0.1)',color:'inherit',cursor:'pointer',borderRadius:'4px',fontSize:'10px',fontFamily:'inherit'}}>CLOSE</button>
                    </div>
                  </>
                )}
              </div>
          ...
          {/* ── PHASE 6.3: PIPELINE LOG ── */}
          {pipelineLog.length > 0 && (
            <div style={{position:'fixed',bottom:'20px',right:'20px',width:'320px',background:'rgba(8,8,16,0.95)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'8px',padding:'12px',zIndex:900,fontFamily:"'JetBrains Mono',monospace",fontSize:'9px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                <span style={{opacity:0.4,letterSpacing:'2px'}}>PIPELINE {pipelineRunning ? '▶ RUNNING' : '■ DONE'}</span>
                {!pipelineRunning && <button onClick={()=>setPipelineLog([])} style={{background:'transparent',border:'none',color:'inherit',opacity:0.3,cursor:'pointer',fontSize:'10px'}}>✕</button>}
              </div>
              <div style={{overflowY:'auto',maxHeight:'160px',display:'flex',flexDirection:'column',gap:'2px'}}>
                {pipelineLog.map((l,i)=>(
                  <div key={i} style={{opacity:0.7,lineHeight:'1.6',color:l.includes('✗')||l.includes('failed')?'#ff435a':l.includes('✓')||l.includes('complete')?'#10b981':'inherit'}}>{l}</div>
                ))}
                {pipelineRunning && <div style={{opacity:0.4}}>▋</div>}
              </div>
            </div>
          )}

          {/* ── TEMPLATES ── */}
          <FloatingPanel title="WORKSPACE TEMPLATES" isOpen={showTemplates} onClose={()=>setShowTemplates(false)} defaultX={Math.max(70,window.innerWidth/2-240)} defaultY={Math.max(50,window.innerHeight/2-280)} defaultW={480} defaultH={520}>
            <div style={{padding:'10px 14px 4px',opacity:0.35,fontSize:'9px',letterSpacing:'1px'}}>Click a template to scaffold it into the current workspace.</div>
            <div style={{flex:1,overflowY:'auto',padding:'8px'}}>
              {(templates.length===0?[
                {id:'fastapi-service',name:'FastAPI Service',description:'FastAPI + Pydantic + SQLAlchemy',icon:'⚡',nodeCount:4},
                {id:'react-component',name:'React Component',description:'TS component + hook + CSS + test',icon:'⚛',nodeCount:4},
                {id:'data-pipeline',name:'Data Pipeline',description:'Source → Transform → Sink',icon:'⟳',nodeCount:5},
                {id:'cli-tool',name:'CLI Tool',description:'argparse + commands + utils',icon:'$_',nodeCount:3},
                {id:'rest-client',name:'REST Client',description:'HTTP client explorer + auth',icon:'↗',nodeCount:3},
              ]:templates).map(tpl=>(
                <div key={tpl.id} className="list-item" style={{padding:'14px',borderRadius:'6px',marginBottom:'6px',border:'1px solid rgba(255,255,255,0.06)',cursor:'pointer',display:'flex',gap:'12px',alignItems:'center',transition:'all 0.15s'}}
                  ...
                  onClick={async()=>{
                    if (!token||!workspaceId){setShowTemplates(false);return;}
                    try {
                      await apiFetch(`/api/workspaces/${workspaceId}`,{method:'POST',body:JSON.stringify({templateId:tpl.id})},token);
                      const data = await apiFetch(`/api/nodes/${workspaceId}`,{},token);
                      const apiNodes = normalizeApiNodes(data);
                      if (apiNodes.length > 0) {
                        nodesRef.current=apiNodes.map(n=>({id:n.id,label:n.label,type:n.type||'function',isMain:n.isMain||false,x:n.position?.x||(Math.random()-.5)*400,y:n.position?.y||(Math.random()-.5)*300,z:0,vx:0,vy:0,themeIdx:n.themeIdx??0,classId:n.groupId||null,code:n.code||'',modified:false,language:n.language||'python',createdAt:n.createdAt?new Date(n.createdAt).getTime():Date.now()}));
                        forceRender({});
                      }
                      setShowTemplates(false);
                    } catch(e){console.warn('Template apply failed:',e.message);}
                  }}>
                  <div style={{fontSize:'22px',flexShrink:0,width:'38px',height:'38px',borderRadius:'7px',background:'rgba(255,255,255,0.04)',display:'flex',alignItems:'center',justifyContent:'center'}}>{tpl.icon||'⬡'}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:'12px',fontWeight:'bold',letterSpacing:'0.5px'}}>{tpl.name}</div>
                    <div style={{fontSize:'10px',opacity:0.4,marginTop:'3px'}}>{tpl.description}</div>
                    <div style={{fontSize:'8px',opacity:0.25,marginTop:'4px',letterSpacing:'0.5px'}}>{tpl.nodeCount||'?'} nodes</div>
                  </div>
          ...
          <div className="main-view">
            ...
            <div className="graph-mode-bar">
              ...
              <button className={`graph-mode-btn ${edgeMode==='join'?'active-join':''}`}
                ...
                title="Join two nodes with an edge (J)">
                {edgeMode==='join' && <span className="join-pulse"/>}
                {edgeMode==='join' ? (joinFirstNode ? '→ CLICK TARGET' : '→ CLICK SOURCE') : '⟶ JOIN'}
              </button>
              {edgeMode==='join' && (
                <select value={joinEdgeType} onChange={e=>setJoinEdgeType(e.target.value)}
                  style={{fontSize:'9px',padding:'3px 6px',background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.12)',color:'inherit',borderRadius:'3px',cursor:'pointer',fontFamily:'inherit',letterSpacing:'0.5px'}}>
                  {['default','imports','calls','data-flow','inherits','test'].map(t=>(
              ...
              <button className={`graph-mode-btn ${edgeMode==='cut'?'active-cut':''}`}
                ...
                title="Remove an edge by clicking it (X)">
                {edgeMode==='cut' && <span className="cut-pulse"/>}
                {edgeMode==='cut' ? '✂ HOVER EDGE' : '✂ CUT'}
              </button>
              <div style={{width:'1px',height:'14px',background:'rgba(255,255,255,0.08)'}}/>
              <button className="graph-mode-btn" onClick={runLayout} title="Auto-layout graph with Dagre (L)"
                style={{color:'rgba(200,200,255,0.6)',borderColor:'rgba(130,130,200,0.2)'}}>
                ⟴ LAYOUT
              </button>
              {edgeMode && (
                <button className="graph-mode-btn" style={{color:'rgba(255,255,255,0.25)',fontSize:'8px',letterSpacing:'0.5px'}} onClick={()=>{setEdgeMode(null);setJoinFirstNode(null);}}>
                  ESC <span style={{opacity:0.4,fontFamily:'system-ui',marginLeft:'1px'}}>⎋</span>
                </button>
              )}
            </div>

            <Graph3D
              nodesRef={nodesRef} edgesRef={edgesRef} groupsRef={groupsRef}
              nodeExecStatus={nodeExecStatus} peers={peers}
              hoveredNodeId={hoveredNodeId} setHoveredNodeId={setHoveredNodeId}
              edgeMode={edgeMode} joinFirstNode={joinFirstNode}
              ...
              handleEdgeClickInMode={handleEdgeClickInMode}
              setNodeColorPicker={setNodeColorPicker}
              runNode={runNode} activeTabId={activeTabId}
              socketRef={socketRef} workspaceId={workspaceId}
              operatorId={operatorId} sessionIdRef={sessionIdRef}
              AVATAR_ACCENTS={AVATAR_ACCENTS}
              showCreateNode={showCreateNode} setShowCreateNode={setShowCreateNode}
            />
            ...
            {/* Minimap removed — use 3D orbit to navigate */}
        ...
        <div className="status-bar">
          <div className="status-group">
            ...
            <span style={{opacity:0.3}}>|</span><span style={{opacity:0.6}}>{themeMode.toUpperCase()}</span>
          ...
          <div className="status-group">
            <span style={{color:'#ffc410',fontWeight:'bold',fontSize:'10px'}}>{workspaceName||'WORKSPACE'}</span>
            <span style={{opacity:0.3}}>|</span><span style={{color: modifiedNodes.length>0 ? '#ffc410' : '#10b981', fontWeight: modifiedNodes.length>0 ? 'bold' : 'normal', fontSize:'9px'}}>{modifiedNodes.length>0?`● ${modifiedNodes.length} unsaved`:'✓ all saved'}</span>
            <span style={{opacity:0.3}}>|</span><span style={{opacity:0.4,cursor:'pointer'}} onClick={()=>setShowCmd(true)}>⌘P</span>
            <span style={{opacity:0.3}}>|</span><span style={{opacity:0.4}}>{clockStr}</span>
          </div>
        </div>
                  ...
                  <div className="grp-editor-body">
                    ...
                    <div style={{flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0}}>
                      <CodeEditor
                        ...
                        onRun={token ? runNode : null}
                        execOutput={nodeOutputs[activeTabNode.id] || ''}
                        execStatus={nodeExecStatus[activeTabNode.id] || 'idle'}
                        ydoc={ydocRef.current}
                        awareness={awarenessRef.current}
                        onChange={code => {
                          const node = nodesRef.current.find(n => n.id === activeTabNode.id);
                          if (node) { node.code = code; node.modified = true; forceRender({}); }
                          // Clear exec status on edit
                          setNodeExecStatus(s => ({...s, [activeTabNode.id]: 'idle'}));
                          // Debounced emit to backend
                          if (activeTabNode._editTimer) clearTimeout(activeTabNode._editTimer);
                          activeTabNode._editTimer = setTimeout(() => {
                            emitNodeEvent('NODE_EDITED', { nodeId: activeTabNode.id, code, language: activeTabNode.language || 'python' });
                          }, 800);
                        }}
                      />
                    </div>
...
function IDEWithCmd({ initialTheme, initialAvatar, token, operatorId, operatorName, workspaceId, workspaceName, onLogout }) {
  const [showCmd, setShowCmd] = useState(false);
  ...
  return (
    <>
      <IDE initialTheme={initialTheme} initialAvatar={initialAvatar}
        token={token} operatorId={operatorId} operatorName={operatorName}
        workspaceId={workspaceId} workspaceName={workspaceName} onLogout={onLogout}/>
...
// ── Workspace picker ──────────────────────────────────────────────────────
function WorkspacePicker({ token, operatorId, operatorName, avatar, theme, onSelect }) {
  ...
  useEffect(() => {
    apiFetch('/api/workspaces', {}, token)
      .then(data => { setWorkspaces(Array.isArray(data) ? data : (data.workspaces || [])); setLoading(false); })
      .catch(() => { setLoading(false); setError('Could not load workspaces'); });
  }, [token]);
  ...
  const createWorkspace = async () => {
    ...
    setCreating(true);
    try {
      const result = await apiFetch('/api/workspaces', { method:'POST', body: JSON.stringify({ name: newName.trim() }) }, token);
      onSelect(result.workspaceId, result.name || newName.trim());
    ...
    <div className="login-container">
      <div className="login-box" style={{maxWidth:'520px'}}>
        <div style={{textAlign:'center'}}>
          <div style={{fontSize:'10px',opacity:0.25,letterSpacing:'4px',marginBottom:'8px',fontFamily:"'JetBrains Mono',monospace"}}>03 // SELECT WORKSPACE</div>
          <div style={{fontSize:'11px',opacity:0.3,letterSpacing:'2px'}}>FORBIDDEN</div>
        </div>

        {loading ? (
          <div style={{textAlign:'center',opacity:0.3,fontSize:'11px',letterSpacing:'2px'}}>LOADING...</div>
        ) : (
          <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
            {workspaces.length === 0 && !error && (
              <div style={{textAlign:'center',opacity:0.25,fontSize:'10px',letterSpacing:'1px',padding:'12px 0'}}>No workspaces yet — create one below</div>
            )}
            {workspaces.map(ws => (
              <div key={ws.workspaceId || ws._id} onClick={() => onSelect(ws.workspaceId || ws._id, ws.name)}
                style={{padding:'14px 18px',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'6px',cursor:'pointer',background:'rgba(255,255,255,0.02)',transition:'all 0.15s',display:'flex',alignItems:'center',gap:'12px'}}
                onMouseEnter={e=>{ e.currentTarget.style.borderColor='rgba(16,185,129,0.3)'; e.currentTarget.style.background='rgba(16,185,129,0.04)'; }}
                ...
                <div>
                  <div style={{fontSize:'12px',fontWeight:600,letterSpacing:'1px'}}>{ws.name}</div>
                  <div style={{fontSize:'9px',opacity:0.3,letterSpacing:'0.5px',marginTop:'2px',fontFamily:"'JetBrains Mono',monospace"}}>{ws.workspaceId || ws._id}</div>
                </div>
                <span style={{marginLeft:'auto',fontSize:'11px',opacity:0.25}}>→</span>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{borderTop:'1px solid rgba(255,255,255,0.06)',paddingTop:'16px'}}>
          <div style={{fontSize:'9px',opacity:0.3,letterSpacing:'2px',marginBottom:'10px'}}>CREATE NEW WORKSPACE</div>
          <div style={{display:'flex',gap:'8px'}}>
            <input
              ...
              style={{flex:1}}
            />
            <button className="theme-btn cyber" disabled={!newName.trim()||creating} onClick={createWorkspace}
              style={{padding:'10px 18px',fontSize:'11px',letterSpacing:'1px',minWidth:'unset'}}>
              {creating ? '...' : 'CREATE'}
            </button>
          </div>
          {error && <div style={{color:'#ff435a',fontSize:'10px',marginTop:'8px',opacity:0.8}}>{error}</div>}
        </div>
      </div>
    </div>
  );
...
// ── Main bootloader with real auth ────────────────────────────────────────
// ── 3D GRAPH CANVAS ──────────────────────────────────────────────────────
function Graph3D({ nodesRef, edgesRef, groupsRef, nodeExecStatus, peers,
  hoveredNodeId, setHoveredNodeId, edgeMode, joinFirstNode,
  hoveredEdgeId, setHoveredEdgeId,
  openNodeInEditor, handleNodeClickInMode, handleEdgeClickInMode,
  setNodeColorPicker, runNode, activeTabId,
  socketRef, workspaceId, operatorId, sessionIdRef, AVATAR_ACCENTS,
  showCreateNode, setShowCreateNode,
}) {
  const mountRef = useRef(null);
  const T3 = useRef({}); // {scene, cam, renderer}
  const nodeMeshMap = useRef({}); // nodeId -> {group, sphere, halo}
  const edgeLineMap = useRef({});  // edgeId -> THREE.Line
  const labelRefs = useRef({});    // nodeId -> DOM element
  const labelContainerRef = useRef(null);
  const timeLabelRefs = useRef({}); // dateStr -> DOM element
  const timeLabelContainerRef = useRef(null);
  const orbit = useRef({ theta: 0.55, phi: 1.05, radius: 820, tx: 0, ty: 0, tz: 0 });
  const ptr = useRef({ down: false, btn: -1, lx: 0, ly: 0, nodeId: null, dragged: false });
  const rcRef = useRef(null);
  const rafRef = useRef(null);
  const timePlaneMeshes = useRef({}); // dateStr -> THREE.Mesh
  const [timePlanes, setTimePlanes] = useState([]); // [{z, label, dateStr}]
  const hasThree = !!THREE;

  const ECOLS = { default:0x3a3a6a, imports:0x89ddff, calls:0x10b981, 'data-flow':0xffc410, inherits:0xbb9af7, test:0xff435a, dependency:0x3a3a6a, data:0xffc410, reference:0x8888ff };

  // Compute time epoch from nodes and build plane list
  const computeTimePlanes = useCallback((nodes) => {
    if (!nodes.length) return [];
    const timestamps = nodes.map(n=>n.createdAt||Date.now());
    const epoch = Math.min(...timestamps);
    const latest = Math.max(...timestamps);
    const DAY_MS = 24*60*60*1000;
    // Generate one plane per calendar day in range, plus "TODAY"
    const startDay = new Date(epoch); startDay.setHours(0,0,0,0);
    const endDay = new Date(latest+DAY_MS); endDay.setHours(0,0,0,0);
    const planes = [];
    let d = new Date(startDay);
    while (d <= endDay) {
      const ts = d.getTime();
      const z = timeToZ(ts, epoch);
      const isToday = new Date().toDateString() === d.toDateString();
      const label = isToday ? 'TODAY' : d.toLocaleDateString('en-US',{month:'short',day:'numeric'});
      const dateStr = d.toISOString().slice(0,10);
      planes.push({ z, label, dateStr, ts, isToday });
  ...
  useEffect(() => {
    ...
    if (!mount) return;
    const T = THREE;
    rcRef.current = new T.Raycaster();

    const scene = new T.Scene();
    const cam = new T.PerspectiveCamera(55, mount.clientWidth / mount.clientHeight, 1, 12000);
    const renderer = new T.WebGLRenderer({ antialias: true, alpha: true });
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    renderer.setSize(mount.clientWidth, mount.clientHeight);
    renderer.setClearColor(0x000000, 0);
    mount.appendChild(renderer.domElement);
    T3.current = { scene, cam, renderer };

    // Lights
    scene.add(new T.AmbientLight(0xffffff, 0.22));
    [[0x10b981,2.2,[400,400,400]],[0x4285f4,1.8,[-400,-300,200]],[0xbb9af7,1.4,[0,-400,-300]]].forEach(([c,i,p])=>{
      const l = new T.PointLight(c,i,1600); l.position.set(...p); scene.add(l);
    });

    // Starfield
    const sp = new Float32Array(3000*3);
    for(let i=0;i<sp.length;i++) sp[i]=(Math.random()-.5)*8000;
    const sg = new T.BufferGeometry(); sg.setAttribute('position', new T.BufferAttribute(sp,3));
    scene.add(new T.Points(sg, new T.PointsMaterial({ color:0xffffff, size:1.1, transparent:true, opacity:0.35 })));

    // Z-axis "time rail" — a thin line running through the centre along Z
    const axisGeo = new T.BufferGeometry().setFromPoints([
      new T.Vector3(0,-220,-1000), new T.Vector3(0,-220,1000)
    ]);
    scene.add(new T.Line(axisGeo, new T.LineBasicMaterial({ color:0x10b981, transparent:true, opacity:0.15 })));
    ...
    const animate = () => {
      ...
      // Recompute epoch each frame (handles new nodes)
      const epoch = nodes.length
        ? nodes.reduce((m,n)=>Math.min(m,n.createdAt||Date.now()),Date.now())
        : Date.now();

      // ── Time-plane slabs ────────────────────────────────────────────────
      const planes = computeTimePlanes(nodes);
      // Sync plane meshes
      const pseen = new Set();
      planes.forEach(pl => {
        pseen.add(pl.dateStr);
        if (!timePlaneMeshes.current[pl.dateStr]) {
          const geo = new T.PlaneGeometry(800, 500);
          const isToday = pl.isToday;
          const mat = new T.MeshBasicMaterial({
            color: isToday ? 0x10b981 : 0x4285f4,
            transparent: true, opacity: isToday ? 0.055 : 0.025,
            side: T.DoubleSide, depthWrite: false,
          });
          const mesh = new T.Mesh(geo, mat);
          mesh.rotation.x = Math.PI / 2; // lay flat in XY
          mesh.rotation.x = 0; mesh.rotation.y = 0; mesh.rotation.z = 0;
          // Stand vertically, face Z direction
          mesh.position.set(0, 0, pl.z);
          scene.add(mesh);
          timePlaneMeshes.current[pl.dateStr] = mesh;

          // Grid lines on the plane
          const gridHelper = new T.GridHelper(600, 8, isToday ? 0x10b981 : 0x2a3a5a, isToday ? 0x10b981 : 0x1a2040);
          gridHelper.rotation.x = Math.PI / 2;
          gridHelper.position.set(0, 0, pl.z);
          gridHelper.material.transparent = true;
          gridHelper.material.opacity = isToday ? 0.18 : 0.08;
          scene.add(gridHelper);
          timePlaneMeshes.current[pl.dateStr + '_grid'] = gridHelper;
        }
      });
      Object.keys(timePlaneMeshes.current).forEach(k => {
        const base = k.replace('_grid','');
        if (!pseen.has(base)) {
          scene.remove(timePlaneMeshes.current[k]);
          delete timePlaneMeshes.current[k];
        }
      });
      ...
      nodes.forEach(n => {
        seen.add(n.id);
        const accent = AVATAR_ACCENTS[n.themeIdx % AVATAR_ACCENTS.length];
        if (!nodeMeshMap.current[n.id]) {
          const col = new T.Color(accent);
          const sphere = new T.Mesh(
            new T.SphereGeometry(16, 24, 24),
            new T.MeshPhongMaterial({ color: col, emissive: col.clone().multiplyScalar(0.28), shininess: 130 })
          );
          const halo = new T.Mesh(
            new T.SphereGeometry(27, 14, 14),
            new T.MeshBasicMaterial({ color: col, transparent: true, opacity: 0.07, side: T.BackSide })
          );
          // Drop line to time plane
          const lineGeo = new T.BufferGeometry().setFromPoints([new T.Vector3(0,0,0), new T.Vector3(0,-180,0)]);
          const dropLine = new T.Line(lineGeo, new T.LineBasicMaterial({ color: new T.Color(accent), transparent:true, opacity: 0.18 }));
          const grp = new T.Group();
          grp.add(sphere, halo, dropLine);
          scene.add(grp);
          nodeMeshMap.current[n.id] = { grp, sphere, halo, dropLine, smat: sphere.material, hmat: halo.material, dmat: dropLine.material };
        }
        const { grp, sphere, smat, hmat, dmat } = nodeMeshMap.current[n.id];
        const nz = timeToZ(n.createdAt||Date.now(), epoch);
        grp.position.set(n.x, n.y, nz);
        smat.color.set(accent); smat.emissive.set(accent); smat.emissive.multiplyScalar(0.28);
        hmat.color.set(accent); dmat.color.set(accent);
        const st = nodeExecStatus[n.id];
        const isHov = hoveredNodeId === n.id, isJoin = joinFirstNode === n.id;
        if (st==='running') { smat.emissive.setHex(0xffc410); smat.emissiveIntensity=0.9; hmat.color.setHex(0xffc410); hmat.opacity=0.2; }
        else if (st==='success') { smat.emissive.setHex(0x10b981); smat.emissiveIntensity=0.7; hmat.opacity=0.1; }
        else if (st==='error') { smat.emissive.setHex(0xff435a); smat.emissiveIntensity=0.8; hmat.color.setHex(0xff435a); hmat.opacity=0.18; }
        else { smat.emissiveIntensity=0.28; hmat.opacity=0.07; }
        if (isHov||isJoin) { smat.emissiveIntensity=Math.max(smat.emissiveIntensity,0.6); hmat.opacity=Math.max(hmat.opacity,0.2); }
        sphere.scale.setScalar(isHov ? 1.35 : isJoin ? 1.2 : n.isMain ? 1.18 : 1.0);
        grp.rotation.y += 0.004;
      });
      Object.keys(nodeMeshMap.current).forEach(id => {
        if (!seen.has(id)) { scene.remove(nodeMeshMap.current[id].grp); delete nodeMeshMap.current[id]; }
      });
      ...
      edges.forEach(e => {
        ...
        if (!src||!tgt) return;
        eseen.add(e.id);
        const sz = timeToZ(src.createdAt||Date.now(), epoch);
        const tz2 = timeToZ(tgt.createdAt||Date.now(), epoch);
        const pts = [new T.Vector3(src.x,src.y,sz), new T.Vector3(tgt.x,tgt.y,tz2)];
        const et = e.edgeType||'default', col = ECOLS[et]??ECOLS.default;
        const isErr = nodeExecStatus[src.id]==='error' && (et==='calls'||et==='data-flow');
        const isHovEdge = hoveredEdgeId===e.id && edgeMode==='cut';
        // Cross-time edges get a dashed "time-travel" look
        const isTimeEdge = Math.abs(sz - tz2) > 20;
        if (!edgeLineMap.current[e.id]) {
          const geo = new T.BufferGeometry().setFromPoints(pts);
          const mat = new T.LineBasicMaterial({ color: isErr?0xff435a:col, transparent:true, opacity: isTimeEdge ? 0.5 : 0.75 });
          const line = new T.Line(geo, mat); scene.add(line);
          edgeLineMap.current[e.id] = line;
        } else {
          edgeLineMap.current[e.id].geometry.setFromPoints(pts);
          edgeLineMap.current[e.id].material.color.setHex(isHovEdge?0xff435a:isErr?0xff435a:col);
          edgeLineMap.current[e.id].material.opacity = isHovEdge ? 1.0 : isTimeEdge ? 0.5 : 0.75;
        }
      });
      Object.keys(edgeLineMap.current).forEach(id => {
        if (!eseen.has(id)) { scene.remove(edgeLineMap.current[id]); delete edgeLineMap.current[id]; }
      });
      ...
      if (labelContainerRef.current) {
        nodes.forEach(n => {
          ...
          if (!el) return;
          const nz = timeToZ(n.createdAt||Date.now(), epoch);
          const v = new T.Vector3(n.x, n.y, nz).project(cam);
          if (v.z > 1) { el.style.display='none'; return; }
          el.style.display='';
          el.style.left = ((v.x*0.5+0.5)*w)+'px';
          el.style.top  = ((-v.y*0.5+0.5)*h+23)+'px';
        });
      }
      // ── Time plane date labels ──────────────────────────────────────────
      if (timeLabelContainerRef.current) {
        planes.forEach(pl => {
          const el = timeLabelRefs.current[pl.dateStr];
          if (!el) return;
          const v = new T.Vector3(-280, 0, pl.z).project(cam);
          if (v.z > 1) { el.style.display='none'; return; }
          el.style.display='';
          el.style.left = ((v.x*0.5+0.5)*w)+'px';
    ...
    return () => {
      ...
      Object.values(nodeMeshMap.current).forEach(({grp})=>scene.remove(grp));
      Object.values(edgeLineMap.current).forEach(l=>scene.remove(l));
      Object.values(timePlaneMeshes.current).forEach(m=>scene.remove(m));
      nodeMeshMap.current={}; edgeLineMap.current={}; timePlaneMeshes.current={};
    };
  }, []);

  // Raycast helper
  const raycastNodes = useCallback((ex, ey) => {
    const T = THREE, rc = rcRef.current, cam = T3.current.cam, mount = mountRef.current;
    if (!T || !rc || !cam || !mount) return null;
    const rect = mount.getBoundingClientRect();
    const ndcX = ((ex-rect.left)/rect.width)*2-1, ndcY = -((ey-rect.top)/rect.height)*2+1;
    rc.setFromCamera({x:ndcX, y:ndcY}, cam);
    const meshes = Object.entries(nodeMeshMap.current).map(([id,{sphere}])=>({id,mesh:sphere}));
    const hits = rc.intersectObjects(meshes.map(m=>m.mesh));
    if (!hits.length) return null;
    return meshes.find(m=>m.mesh===hits[0].object)?.id || null;
  }, []);

  const onPointerDown = useCallback((e) => {
    if (e.target.closest('.g3d-lbl,.floating-panel,.graph-mode-bar')) return;
    e.currentTarget.setPointerCapture(e.pointerId);
    ptr.current = { down:true, btn:e.button, lx:e.clientX, ly:e.clientY, nodeId:null, dragged:false };
    if (e.button===0) {
      const nid = raycastNodes(e.clientX, e.clientY);
      if (nid) ptr.current.nodeId = nid;
    }
  }, [raycastNodes]);

  const onPointerMove = useCallback((e) => {
    const dx = e.clientX - ptr.current.lx, dy = e.clientY - ptr.current.ly;
    if (!ptr.current.down) {
      // Hover
      const nid = raycastNodes(e.clientX, e.clientY);
    ...
    if (Math.abs(dx)>1||Math.abs(dy)>1) ptr.current.dragged = true;

    if (ptr.current.nodeId && ptr.current.btn===0 && !edgeMode) {
      const T = THREE, rc = rcRef.current, cam = T3.current.cam, mount = mountRef.current;
      const rect = mount.getBoundingClientRect();
      const ndcX = ((e.clientX-rect.left)/rect.width)*2-1, ndcY = -((e.clientY-rect.top)/rect.height)*2+1;
      rc.setFromCamera({x:ndcX,y:ndcY},cam);
      const node = nodesRef.current.find(n=>n.id===ptr.current.nodeId);
      if (node) {
        // Drag only in XY — Z is locked to createdAt (time is immutable)
        const epoch = nodesRef.current.reduce((m,n)=>Math.min(m,n.createdAt||Date.now()),Date.now());
        const nz = timeToZ(node.createdAt||Date.now(), epoch);
        const plane = new T.Plane(new T.Vector3(0,0,1), -nz);
        const hit = new T.Vector3();
        if (rc.ray.intersectPlane(plane, hit)) { node.x=hit.x; node.y=hit.y; node.vx=0; node.vy=0; }
      }
    } else if (!ptr.current.nodeId) {
      if (ptr.current.btn===0) {
        // Orbit
        orbit.current.theta -= dx * 0.007;
        orbit.current.phi = Math.max(0.08, Math.min(Math.PI-0.08, orbit.current.phi - dy * 0.007));
      } else if (ptr.current.btn===2||ptr.current.btn===1) {
        // Pan — move target in camera-right and camera-up
        const T = THREE, cam = T3.current.cam;
        const right = new T.Vector3(); cam.getWorldDirection(new T.Vector3()).cross(new T.Vector3(0,1,0)).normalize();
        orbit.current.tx -= dx * 0.55;
        orbit.current.ty += dy * 0.55;
      }
    }
    ptr.current.lx = e.clientX; ptr.current.ly = e.clientY;
  }, [edgeMode, raycastNodes, nodesRef]);

  const onPointerUp = useCallback((e) => {
    if (!ptr.current.dragged && ptr.current.btn===0) {
      const nid = ptr.current.nodeId || raycastNodes(e.clientX, e.clientY);
      if (nid) {
        if (edgeMode==='join') handleNodeClickInMode(nid);
        else if (!edgeMode) openNodeInEditor(nid);
      }
    }
    if (ptr.current.nodeId && ptr.current.dragged) {
      const node = nodesRef.current.find(n=>n.id===ptr.current.nodeId);
      if (node && socketRef.current?.connected) {
        socketRef.current.emit('node:edit', {
          workspaceId, nodeId:node.id, position:{x:node.x,y:node.y},
          operatorId, sessionId:sessionIdRef.current,
          ...(makeClientEventId() ? { clientEventId: makeClientEventId() } : {}),
        });
      }
    }
    ptr.current.down = false; ptr.current.nodeId = null;
  }, [edgeMode, raycastNodes, handleNodeClickInMode, openNodeInEditor, nodesRef]);

  const onWheel = useCallback((e) => {
    if (e.target.closest('.floating-panel')) return;
    e.preventDefault();
    orbit.current.radius = Math.max(80, Math.min(3500, orbit.current.radius * (e.deltaY>0?1.09:0.92)));
  }, []);
  ...
  return (
    <div style={{position:'relative',flex:1,overflow:'hidden'}}
      ...
      onPointerLeave={()=>{ptr.current.down=false;ptr.current.nodeId=null;setHoveredNodeId(null);}}
      onWheel={onWheel} onContextMenu={e=>e.preventDefault()}>

      {!hasThree && (
        <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',zIndex:20,background:'rgba(5,5,10,0.9)'}}>
          <div style={{padding:'14px 18px',border:'1px solid rgba(255,67,90,0.25)',background:'rgba(255,67,90,0.06)',borderRadius:'8px',fontFamily:"'JetBrains Mono', monospace",fontSize:'10px',letterSpacing:'1px',color:'#ff8080'}}>
            GRAPH RENDERER NOT LOADED
          </div>
        </div>
      )}
      ...
      {/* HTML label overlay — positions updated imperatively in RAF */}
      <div ref={labelContainerRef} style={{position:'absolute',inset:0,pointerEvents:'none'}}>
        {nodes.map(n => {
          const accent = AVATAR_ACCENTS[n.themeIdx % AVATAR_ACCENTS.length];
          const st = nodeExecStatus[n.id];
          const nodePeers = Object.values(peers).filter(p=>p.nodeId===n.id);
          const dateStr = n.createdAt ? new Date(n.createdAt).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'2-digit'}) : '';
          return (
            <div key={n.id} className="g3d-lbl" ref={el=>{if(el)labelRefs.current[n.id]=el;else delete labelRefs.current[n.id];}}
              style={{position:'absolute',transform:'translateX(-50%)',pointerEvents:'auto',zIndex:5,cursor:'pointer',userSelect:'none'}}>
              <div style={{background:'rgba(6,6,18,0.9)',border:`1px solid ${accent}55`,borderRadius:'4px',padding:'2px 8px',
                fontSize:'9px',color:accent,fontFamily:"'JetBrains Mono',monospace",letterSpacing:'0.5px',whiteSpace:'nowrap',
                display:'flex',alignItems:'center',gap:'5px',boxShadow:`0 0 14px ${accent}22`}}>
                <span style={{width:'6px',height:'6px',borderRadius:'50%',background:accent,flexShrink:0,boxShadow:`0 0 6px ${accent}`}}/>
                <span onClick={e=>{e.stopPropagation();if(edgeMode==='join')handleNodeClickInMode(n.id);else openNodeInEditor(n.id);}}>
                  {n.label}{n.modified&&<span style={{color:'#ffc410',marginLeft:'3px'}}>●</span>}
                </span>
                {st==='running'&&<span style={{color:'#ffc410',animation:'pulse 0.8s infinite',fontSize:'8px'}}>▶</span>}
                {st==='success'&&<span style={{color:'#10b981',fontSize:'8px'}}>✓</span>}
                {st==='error'&&<span style={{color:'#ff435a',fontSize:'8px'}}>✗</span>}
                <span style={{opacity:0.4,fontSize:'8px',cursor:'pointer'}} onPointerDown={e=>e.stopPropagation()}
                  onClick={e=>{e.stopPropagation();runNode(n.id);}}>▶</span>
                <span style={{opacity:0.3,fontSize:'8px',cursor:'pointer'}} onPointerDown={e=>e.stopPropagation()}
                  onClick={e=>{e.stopPropagation();const rect=e.currentTarget.getBoundingClientRect();setNodeColorPicker({nodeId:n.id,x:rect.left,y:rect.bottom+6});}}>●</span>
              </div>
              {dateStr && <div style={{fontSize:'7px',opacity:0.3,fontFamily:"'JetBrains Mono',monospace",textAlign:'center',marginTop:'1px',letterSpacing:'0.3px',color:'rgba(255,255,255,0.4)'}}>{dateStr}</div>}
              {nodePeers.length>0&&(
                <div style={{display:'flex',justifyContent:'center',gap:'-2px',marginTop:'2px',pointerEvents:'none'}}>
                  {nodePeers.slice(0,3).map((p,i)=>(
                    <div key={i} title={p.name} style={{width:'12px',height:'12px',borderRadius:'50%',background:p.color||'#10b981',border:'1.5px solid rgba(8,8,16,0.9)',marginLeft:i>0?'-4px':'0',fontSize:'6px',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:'bold',boxShadow:`0 0 5px ${p.color||'#10b981'}88`}}>
      ...
      {/* Time-plane date labels — positioned imperatively in RAF */}
      <div ref={timeLabelContainerRef} style={{position:'absolute',inset:0,pointerEvents:'none'}}>
        {computeTimePlanes(nodes).map(pl => (
          <div key={pl.dateStr} ref={el=>{if(el)timeLabelRefs.current[pl.dateStr]=el;else delete timeLabelRefs.current[pl.dateStr];}}
            style={{position:'absolute',transform:'translate(-50%,-50%)',whiteSpace:'nowrap'}}>
            <div style={{
              fontSize:'9px',fontFamily:"'JetBrains Mono',monospace",letterSpacing:'2px',
              color: pl.isToday ? '#10b981' : 'rgba(66,133,244,0.55)',
              background: pl.isToday ? 'rgba(16,185,129,0.08)' : 'rgba(0,0,0,0.55)',
              border: `1px solid ${pl.isToday ? 'rgba(16,185,129,0.3)' : 'rgba(66,133,244,0.2)'}`,
              borderRadius:'3px',padding:'2px 8px',
              textShadow: pl.isToday ? '0 0 12px #10b981' : 'none',
            }}>
              {pl.label}
            </div>
          </div>
      ...
      {nodes.length===0&&(
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',pointerEvents:'none',zIndex:2}}>
          <div style={{textAlign:'center',opacity:0.08,userSelect:'none'}}>
            <div style={{fontSize:'80px',fontFamily:"'JetBrains Mono',monospace",lineHeight:1}}>⬡</div>
            <div style={{fontSize:'13px',letterSpacing:'5px',marginTop:'14px',fontFamily:"'JetBrains Mono',monospace",fontWeight:900}}>FORBIDDEN</div>
            <div style={{fontSize:'9px',letterSpacing:'2.5px',marginTop:'5px',opacity:0.7}}>EMPTY WORKSPACE</div>
          </div>
          <div style={{marginTop:'32px',display:'flex',flexDirection:'column',alignItems:'center',gap:'7px',opacity:0.15,pointerEvents:'auto'}}>
            <button className="btn hdr-pill" onClick={()=>setShowCreateNode(true)} style={{fontSize:'9px',letterSpacing:'1.5px'}}>＋ CREATE FIRST NODE</button>
            <div style={{fontSize:'8px',opacity:0.7,letterSpacing:'1px',fontFamily:"'JetBrains Mono',monospace"}}>OR PRESS N</div>
          </div>
        </div>
      )}

      {/* Controls hint */}
      <div style={{position:'absolute',bottom:'12px',left:'50%',transform:'translateX(-50%)',zIndex:10,display:'flex',gap:'8px',alignItems:'center'}}>
        <div style={{background:'rgba(6,6,18,0.7)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:'6px',padding:'4px 14px',fontSize:'8px',opacity:0.4,letterSpacing:'1px',fontFamily:"'JetBrains Mono',monospace",pointerEvents:'none',whiteSpace:'nowrap'}}>
          DRAG · ORBIT  |  RIGHT DRAG · PAN  |  SCROLL · ZOOM  |  Z-AXIS = TIME
        </div>
        <button style={{padding:'4px 10px',background:'rgba(6,6,18,0.85)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'4px',color:'rgba(255,255,255,0.35)',fontSize:'8px',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace",letterSpacing:'1px'}}
          onClick={()=>{ orbit.current={theta:0.55,phi:1.05,radius:820,tx:0,ty:0,tz:0}; }}>
          ⌖ RESET
        </button>
        <button style={{padding:'4px 10px',background:'rgba(6,6,18,0.85)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'4px',color:'rgba(255,255,255,0.35)',fontSize:'8px',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace",letterSpacing:'1px'}}
          onClick={()=>{ orbit.current.phi=0.12; orbit.current.theta=0; orbit.current.radius=900; }}>
          ↑ TOP
        </button>
        <button style={{padding:'4px 10px',background:'rgba(6,6,18,0.85)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'4px',color:'rgba(16,185,129,0.5)',fontSize:'8px',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace",letterSpacing:'1px',borderColor:'rgba(16,185,129,0.2)'}}
          onClick={()=>{ orbit.current.theta=Math.PI/2; orbit.current.phi=Math.PI/2; orbit.current.radius=900; }}>
          ⟶ TIMELINE
        </button>
      </div>
    </div>
...
// ── Phase 6.2: Read-only Share View ──────────────────────────────────────
function ShareView({ shareToken }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/share/${shareToken}`)
  ...
  if (loading) return (
    <div style={{width:'100vw',height:'100vh',background:'#050505',display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,0.2)',fontFamily:"'JetBrains Mono',monospace",letterSpacing:'3px',fontSize:'11px'}}>
      LOADING SNAPSHOT...
    </div>
  );
  if (error || !data) return (
    <div style={{width:'100vw',height:'100vh',background:'#050505',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',color:'#ff435a',fontFamily:"'JetBrains Mono',monospace',gap:'12px"}}>
      <div style={{fontSize:'40px',opacity:0.3}}>⚠</div>
      <div style={{letterSpacing:'2px',fontSize:'11px'}}>SHARE LINK EXPIRED OR INVALID</div>
      <div style={{fontSize:'9px',opacity:0.3,marginTop:'4px'}}>{error}</div>
    </div>
  );

  const nodes = data.nodes || [];
  const edges = nodes.flatMap(n => (n.edges||[]).map(e => ({ id:`${n.id}-${e.targetId}`, source:n.id, target:e.targetId, edgeType:e.edgeType||'default' })));
  const [activeId, setActiveId] = useState(null);
  const activeNode = nodes.find(n => n.id === activeId);

  return (
    <div style={{width:'100vw',height:'100vh',background:'#080810',color:'#9494b0',fontFamily:"'JetBrains Mono',monospace",display:'flex',flexDirection:'column',overflow:'hidden'}}>
      {/* Header */}
      <div style={{display:'flex',alignItems:'center',gap:'16px',padding:'10px 20px',borderBottom:'1px solid rgba(255,255,255,0.06)',background:'rgba(0,0,0,0.3)',flexShrink:0}}>
        <div style={{fontSize:'13px',fontWeight:900,letterSpacing:'4px',color:'rgba(255,255,255,0.8)'}}>FORBIDDEN</div>
        <div style={{width:'1px',height:'16px',background:'rgba(255,255,255,0.1)'}}/>
        <div style={{fontSize:'9px',opacity:0.35,letterSpacing:'2px'}}>{data.snapshot?.workspaceId || 'SNAPSHOT'}</div>
        <div style={{padding:'2px 10px',background:'rgba(255,67,90,0.1)',border:'1px solid rgba(255,67,90,0.3)',borderRadius:'20px',fontSize:'8px',color:'#ff435a',letterSpacing:'1px'}}>READ ONLY</div>
        <div style={{fontSize:'8px',opacity:0.2,marginLeft:'auto',letterSpacing:'1px'}}>Expires: {data.expiresAt ? new Date(data.expiresAt).toLocaleString() : '24h'}</div>
      </div>
      {/* Body */}
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
        ...
        <div style={{flex:1,position:'relative',overflow:'hidden'}}>
          <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none'}}>
            <defs>
              {['default','imports','calls','data-flow','inherits','test'].map(t => (
                <marker key={t} id={`sh-arrow-${t}`} markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
                  <path d="M0,0 L0,6 L8,3 z" fill={{default:'rgba(255,255,255,0.25)',imports:'#89ddff',calls:'#10b981','data-flow':'#ffc410',inherits:'#bb9af7',test:'#ff435a'}[t]||'rgba(255,255,255,0.25)'}/>
                </marker>
              ))}
            </defs>
            {edges.map(e => {
              ...
              if (!s||!t) return null;
              const mx=(s.position?.x||0+t.position?.x||0)/2, my=(s.position?.y||0+t.position?.y||0)/2-20;
              const COLORS = {default:'rgba(255,255,255,0.18)',imports:'#89ddff',calls:'#10b981','data-flow':'#ffc410',inherits:'#bb9af7',test:'#ff435a'};
              return (
                <path key={e.id} d={`M${s.position?.x||0} ${s.position?.y||0} Q${mx} ${my} ${t.position?.x||0} ${t.position?.y||0}`}
                  fill="none" stroke={COLORS[e.edgeType]||COLORS.default} strokeWidth="1.5" opacity="0.7"
                  markerEnd={e.edgeType!=='default'?`url(#sh-arrow-${e.edgeType})`:undefined}/>
              );
            })}
          </svg>
          <div style={{position:'absolute',inset:0}}>
            {nodes.map(n => {
              const accent = AVATAR_ACCENTS[((n.themeIdx||0))%AVATAR_ACCENTS.length];
              const x = n.position?.x || (Math.random()-0.5)*400 + 400;
              const y = n.position?.y || (Math.random()-0.5)*300 + 300;
              return (
                <div key={n.id} style={{position:'absolute',left:x+'px',top:y+'px',transform:'translate(-50%,-50%)',cursor:'pointer',zIndex:10}}
                  onClick={()=>setActiveId(id=>id===n.id?null:n.id)}>
                  <div style={{width:'40px',height:'40px',borderRadius:'50%',background:'#080810',border:`2px solid ${accent}`,boxShadow:`0 0 12px ${accent}66`,display:'flex',alignItems:'center',justifyContent:'center',transition:'transform 0.15s',transform:activeId===n.id?'scale(1.3)':'scale(1)'}}>
                    <span style={{fontSize:'10px',opacity:0.6}}>{n.type==='class'?'C':n.type==='test'?'T':'ƒ'}</span>
                  </div>
                  <div style={{position:'absolute',top:'44px',left:'50%',transform:'translateX(-50%)',whiteSpace:'nowrap',fontSize:'9px',color:accent,background:'rgba(8,8,16,0.85)',padding:'2px 6px',borderRadius:'3px',border:`1px solid ${accent}33`,pointerEvents:'none'}}>{n.label}</div>
                </div>
              );
            })}
          </div>
        ...
        {/* Code panel */}
        {activeNode && (
          <div style={{width:'400px',flexShrink:0,borderLeft:'1px solid rgba(255,255,255,0.06)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            <div style={{padding:'10px 14px',borderBottom:'1px solid rgba(255,255,255,0.06)',fontSize:'10px',display:'flex',gap:'8px',alignItems:'center',flexShrink:0}}>
              <div style={{width:'8px',height:'8px',borderRadius:'50%',background:AVATAR_ACCENTS[(activeNode.themeIdx||0)%AVATAR_ACCENTS.length],boxShadow:`0 0 6px ${AVATAR_ACCENTS[(activeNode.themeIdx||0)%AVATAR_ACCENTS.length]}`}}/>
              <span style={{opacity:0.8}}>{activeNode.label}</span>
              <span style={{opacity:0.3,fontSize:'9px',marginLeft:'auto'}}>{activeNode.type}</span>
            </div>
            <pre style={{flex:1,margin:0,padding:'14px',overflow:'auto',fontSize:'11px',fontFamily:"'JetBrains Mono',monospace",lineHeight:'1.7',color:'rgba(200,220,200,0.8)',background:'#040408',whiteSpace:'pre-wrap',wordBreak:'break-all',userSelect:'text'}}>{activeNode.code||'(empty)'}</pre>
          </div>
        )}
      </div>
      {/* Footer */}
      <div style={{padding:'6px 20px',borderTop:'1px solid rgba(255,255,255,0.04)',fontSize:'8px',opacity:0.2,letterSpacing:'1px',display:'flex',gap:'20px',flexShrink:0}}>
        <span>{nodes.length} nodes</span>
        <span>{edges.length} edges</span>
        <span style={{marginLeft:'auto'}}>FORBIDDEN — Read-Only Share</span>
      </div>
    </div>
  );
}
...
function Bootloader() {
  const [step, setStep] = useState(() => {
    // Phase 1.1 — Restore session on reload
    try {
      const saved = sessionStorage.getItem('forbidden:session');
      if (saved) {
        const s = JSON.parse(saved);
        if (s.token && s.workspaceId) return 'ide';
      }
    } catch {}
    return 'identity';
  });
  const [theme, setTheme] = useState(() => {
    try { return sessionStorage.getItem('forbidden:theme') || null; } catch { return null; }
  });
  const [avatar, setAvatar] = useState(() => {
    try { const v = sessionStorage.getItem('forbidden:avatar'); return v !== null ? Number(v) : null; } catch { return null; }
  });
  ...
  const [token, setToken] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('forbidden:session') || 'null')?.token || null; } catch { return null; }
  });
  const [operatorId, setOperatorId] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('forbidden:session') || 'null')?.operatorId || ''; } catch { return ''; }
  });
  const [operatorName, setOperatorName] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('forbidden:session') || 'null')?.operatorName || ''; } catch { return ''; }
  });
  const [workspaceId, setWorkspaceId] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('forbidden:session') || 'null')?.workspaceId || ''; } catch { return ''; }
  });
  const [workspaceName, setWorkspaceName] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('forbidden:session') || 'null')?.workspaceName || ''; } catch { return ''; }
  });

  const saveSession = (tok, opId, opName, wsId, wsName) => {
    try { sessionStorage.setItem('forbidden:session', JSON.stringify({ token: tok, operatorId: opId, operatorName: opName, workspaceId: wsId, workspaceName: wsName })); } catch {}
  };

  const handleThemeSelect = (t) => {
    setTheme(t);
    try { sessionStorage.setItem('forbidden:theme', t); } catch {}
    if (avatar !== null) setStep('credentials');
  };
  const handleAvatarSelect = (i) => {
    setAvatar(i);
    try { sessionStorage.setItem('forbidden:avatar', String(i)); } catch {}
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setToken(data.token);
      setOperatorId(data.operator?.id || data.operator?.sub || data.userId || username);
      setOperatorName(data.operator?.name || data.operator?.login || username);
      setStep('workspace');
    } catch (e) {
      setAuthError(e.status === 401 ? 'Invalid credentials' : e.message);
    } finally { setAuthLoading(false); }
  };

  const handleDemoMode = () => {
    setToken('DEMO'); setOperatorId('demo'); setOperatorName('OPERATOR');
    setWorkspaceId('demo'); setWorkspaceName('Demo Workspace'); setStep('ide');
  };

  const handleLogout = () => {
    try { sessionStorage.removeItem('forbidden:session'); } catch {}
    setToken(null); setWorkspaceId(''); setWorkspaceName(''); setStep('identity');
  };

  if (step === 'ide') {
    return <IDEWithCmd initialTheme={theme || 'cyber'} initialAvatar={avatar || 0}
  ...
  if (step === 'workspace' && token) {
    return <WorkspacePicker token={token} operatorId={operatorId} operatorName={operatorName}
      avatar={avatar} theme={theme}
      onSelect={(wsId, wsName) => {
        setWorkspaceId(wsId); setWorkspaceName(wsName); setStep('ide');
        saveSession(token, operatorId, operatorName, wsId, wsName);
      }}/>;
  }
  ...
  return (
    <div className="login-container">
      <div className="login-box">
        <div style={{textAlign:'center'}}>
          <div style={{fontSize:'10px',opacity:0.25,letterSpacing:'4px',marginBottom:'14px',fontFamily:"'JetBrains Mono',monospace"}}>SYSTEM BOOT SEQUENCE</div>
          <h1 className="boot-title" style={{fontSize:'34px',fontWeight:900,letterSpacing:'6px',fontFamily:"'JetBrains Mono',monospace",marginBottom:'6px'}}>FORBIDDEN</h1>
          <div style={{fontSize:'10px',opacity:0.2,letterSpacing:'3px'}}>DUAL THEME GRAPH IDE</div>
        </div>

        {/* Step 1: Avatar */}
        <div className="boot-section" style={{animationDelay:'0.2s',opacity:0,background:'rgba(255,255,255,0.02)',padding:'28px',borderRadius:'6px',border:'1px solid rgba(255,255,255,0.06)'}}>
          <div style={{fontSize:'9px',opacity:0.35,marginBottom:'20px',textAlign:'center',letterSpacing:'3px'}}>01 // SELECT OPERATOR IDENTITY</div>
          <div style={{display:'flex',gap:'14px',justifyContent:'center',flexWrap:'wrap'}}>
            {[0,1,2,3,4,5].map(i=>(
              <div key={i} onClick={()=>handleAvatarSelect(i)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'7px',cursor:'pointer',transition:'transform 0.2s',transform:avatar===i?'translateY(-4px)':'none'}}>
                <CyberAvatar index={i} size={68} selected={avatar===i}/>
                <div style={{fontSize:'8px',letterSpacing:'1px',opacity:avatar===i?1:0.25,color:AVATAR_ACCENTS[i],fontFamily:"'JetBrains Mono',monospace",transition:'opacity 0.2s'}}>{AVATAR_NAMES[i]}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{opacity:avatar!==null?1:0.15,transition:'opacity 0.4s',pointerEvents:avatar!==null?'auto':'none'}}>
          <div style={{fontSize:'9px',opacity:0.35,marginBottom:'14px',textAlign:'center',letterSpacing:'3px'}}>02 // INITIALIZE ENGINE</div>
          <div style={{display:'flex',gap:'16px'}}>
            <button className="theme-btn cyber" disabled={avatar===null} onClick={()=>handleThemeSelect('cyber')}>
              <div style={{fontSize:'20px',fontWeight:'bold',marginBottom:'6px',letterSpacing:'2px'}}>OBSIDIAN</div>
              <div style={{fontSize:'9px',opacity:0.5,letterSpacing:'1px'}}>DARK / CYBERPUNK</div>
            </button>
            <button className="theme-btn brutal" disabled={avatar===null} onClick={()=>handleThemeSelect('brutal')}>
              <div style={{fontSize:'20px',fontWeight:900,marginBottom:'6px',letterSpacing:'2px'}}>FORSAKEN</div>
              <div style={{fontSize:'9px',opacity:0.6,fontWeight:'bold',letterSpacing:'1px'}}>LIGHT / BRUTALIST</div>
            </button>
          </div>
        </div>

        {/* Step 3: Credentials — appears after theme is chosen */}
        {step === 'credentials' && (
          <div style={{borderTop:'1px solid rgba(255,255,255,0.06)',paddingTop:'20px',display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{fontSize:'9px',opacity:0.35,textAlign:'center',letterSpacing:'3px',marginBottom:'4px'}}>03 // AUTHENTICATE</div>
            ...
            {authError && <div style={{color:'#ff435a',fontSize:'10px',opacity:0.8}}>{authError}</div>}
            <div style={{display:'flex',gap:'8px'}}>
              <button className={`theme-btn ${theme}`} style={{flex:1}}
                disabled={!username.trim()||!password.trim()||authLoading} onClick={handleLogin}>
                {authLoading ? 'CONNECTING...' : 'CONNECT'}
              </button>
              <button onClick={handleDemoMode}
                style={{padding:'10px 14px',border:'1px solid rgba(255,255,255,0.1)',background:'transparent',color:'rgba(255,255,255,0.3)',cursor:'pointer',borderRadius:'4px',fontSize:'9px',letterSpacing:'1px',transition:'all 0.2s'}}
            ...
            <div style={{textAlign:'center',fontSize:'9px',opacity:0.2,letterSpacing:'1px'}}>
              No backend? Click DEMO to run offline.
            </div>
          </div>
        )}
      </div>
...
export default function App() {
  // Share-link routing stays at the app root in the bundled build too.
  const urlParams = new URLSearchParams(window.location.search);
  const shareToken = urlParams.get('share');
  return shareToken ? <ShareView shareToken={shareToken}/>
    : <Bootloader />;
}
