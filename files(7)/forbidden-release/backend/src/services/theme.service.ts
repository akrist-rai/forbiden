...
// Re-export built-in Monaco theme data for tests and any tooling that needs the
// canonical in-memory definitions before the DB is seeded.
export const BUILTIN_THEME_DATA = BUILTIN_THEMES;
...
export class ThemeService {
  ...
  static async listForWorkspace(workspaceId: string): Promise<ITheme[]> {
    ...
    }).sort({ builtIn: -1, name: 1 }).lean() as unknown as Promise<ITheme[]>;
  }
